...
Page({
  ...
  selectClip(e: any) {
    ...
    // 跳转到音频播放页面
    wx.navigateTo({
      url: `/packageNav/audio-player/index?regionId=${this.data.regionId}&regionName=${encodeURIComponent(this.data.regionName)}&categoryId=${this.data.categoryId}&categoryName=${encodeURIComponent(this.data.categoryName)}&clipIndex=${index}&allClipsJson=${encodeURIComponent(JSON.stringify(this.data.allClips))}`
    });
  },

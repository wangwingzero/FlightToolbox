...
Page({
  ...
  selectCategory(e: any) {
    ...
    if (category) {
      // 跳转到录音列表页面
      wx.navigateTo({
        url: `/packageNav/recording-clips/index?regionId=${this.data.regionId}&regionName=${encodeURIComponent(this.data.regionName)}&regionFlag=${encodeURIComponent(this.data.regionFlag)}&categoryId=${categoryId}&categoryName=${encodeURIComponent(category.name)}&allClipsJson=${encodeURIComponent(JSON.stringify(category.clips))}`
      });
    }
  },

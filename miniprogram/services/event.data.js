...
function getCategoryById(categoryId) {
    return exports.eventCategories.find(category => category.id === categoryId) || null;
}
/**
 * 根据事件类型ID获取事件类型
 */
function getEventTypeById(eventTypeId) {
    for (const category of exports.eventCategories) {
        const eventType = category.eventTypes.find((type) => type.id === eventTypeId);
        if (eventType) {
            return eventType;
        }
    }

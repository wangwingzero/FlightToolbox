...
Page({
    ...
    // 加载历史记录
    loadReports() {
        const reports = storage_service_1.StorageService.getSavedReports();
    ...
    deleteReport(e) {
        ...
        wx.showModal({
            ...
            success: (res) => {
                if (res.confirm) {
                    const success = storage_service_1.StorageService.deleteReport(report.timestamp);
                    if (success) {
                        wx.showToast({
                            title: '删除成功',
                            icon: 'success'
    ...
    deleteSelectedReport() {
        if (this.data.selectedReport) {
            wx.showModal({
                ...
                success: (res) => {
                    if (res.confirm) {
                        const success = storage_service_1.StorageService.deleteReport(this.data.selectedReport.timestamp);
                        if (success) {
                            wx.showToast({
                                title: '删除成功',
                                icon: 'success'

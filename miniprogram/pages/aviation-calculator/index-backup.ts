...
Page({
  ...
  extractPlainLanguageFromInput(input: string) {
    ...
    // 查找REMARK部分 - 这是性能计算部分的明语说明
    const lines = cleanInput.split(/[\n\r]/).map(line => line.trim()).filter(line => line)
    
    let remarkLines = []
    let foundRemarkSection = false
    
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i]
      console.log('🔍 处理REMARK行:', line)
      
      // 检查是否包含REMARK部分（可能在行的中间）
      if (line.match(/REMARK\s*[\/\.]?/i)) {
        foundRemarkSection = true
        
        // 提取REMARK前的内容（如果包含性能相关信息）
        const beforeRemark = line.split(/REMARK\s*[\/\.]?/i)[0].trim()
        if (beforeRemark && beforeRemark.match(/RWY|TWY|APRON|POOR/i)) {
          remarkLines.push(beforeRemark)
        }
        
        // 提取REMARK后的内容
        const remarkContent = line.replace(/.*REMARK\s*[\/\.]?/i, '').trim()
        if (remarkContent) {
          remarkLines.push(remarkContent)
        }
        
        console.log('🔍 找到REMARK部分:', line)
        console.log('🔍 REMARK前内容:', beforeRemark)
        console.log('🔍 REMARK后内容:', remarkContent)
        continue
      }
      
      // 如果已经找到REMARK部分，继续收集后续的性能相关行
      if (foundRemarkSection) {
        // 检查是否是REMARK部分的延续（性能计算相关内容）
        if (line.match(/RWY|TWY|APRON|UPGRADED|DOWNGRADED|POOR|CONTAMINANT|NOT\s+IN\s+USE|TAKEOFF|SIGNIFICANT|FIRST|SECOND|THIRD|PART/i)) {
          remarkLines.push(line)
          continue
        }
        
        // 如果遇到明显的结束标记（如括号结尾），收集这行后停止
        if (line.match(/\)$/) || (line.match(/\.$/) && line.match(/RWY|TWY|APRON/i))) {
          remarkLines.push(line.replace(/[\)\.]$/, ''))
          break
        }
        
        // 如果这行看起来像是REMARK的延续（包含英文单词但不是纯数据）
        if (line.match(/[A-Z\s]{5,}/) && !line.match(/[\d\/]{5,}/)) {
          remarkLines.push(line)
          continue
        }
      }
      
      // 即使没有REMARK标记，也查找包含性能计算相关关键词的行
      if (!foundRemarkSection && line.match(/(?:UPGRADED|DOWNGRADED|CONTAMINANT|TAKEOFF\s+SIGNIFICANT|FIRST\s+PART|SECOND\s+PART|THIRD\s+PART)/i)) {
        remarkLines.push(line)
        console.log('🔍 收集性能相关行（无REMARK标记）:', line)
        continue
      }
    }
    
    // 将所有收集到的REMARK行合并
    const result = remarkLines.join(' ').trim()
    console.log('🔍 REMARK提取结果 - 收集的行数:', remarkLines.length)
    console.log('🔍 REMARK提取结果 - 各行内容:', remarkLines)
    console.log('🔍 REMARK提取结果 - 最终合并:', result)
    return result
  },

  // 翻译明语说明项目（性能计算部分的REMARK内容）
  // 翻译明语说明项目（情景意识部分的T项内容）
  translatePlainLanguageItems(plainLanguage: string) {
    ...
    console.log('🔍 翻译明语说明项目 - 输入文本:', plainLanguage)
    console.log('🔍 转换为大写:', upperText)
    ...
    // T项：明语说明（包含RWYCC升级、降级、起飞污染物等特殊情况）
    
    // RWYCC升级情况
    const rwyccUpgradedMatches = upperText.match(/RWY\s+(\w+)\s+(?:(FIRST|SECOND|THIRD)\s+PART\s+)?RWYCC\s+UPGRADED/g)
    if (rwyccUpgradedMatches) {
      rwyccUpgradedMatches.forEach(match => {
        const parts = match.match(/RWY\s+(\w+)\s+(?:(FIRST|SECOND|THIRD)\s+PART\s+)?RWYCC\s+UPGRADED/)
        if (parts) {
          const runway = parts[1]
          const part = parts[2]
          let partDesc = ''
          if (part) {
            ...
            partDesc = partMap[part] || part
          }
          
          items.push({
            code: 'T',
            title: '明语说明 - RWYCC升级',
            content: `跑道${runway}${partDesc ? partDesc : ''}的RWYCC已升级`,
            note: '跑道状况已改善，实际摩擦系数可能比报告的RWYCC代码更好。'
          })
        }
      })
    }
    ...
    // RWYCC降级情况
    const rwyccDowngradedMatches = upperText.match(/RWY\s+(\w+)\s+(?:(FIRST|SECOND|THIRD)\s+PART\s+)?RWYCC\s+DOWNGRADED/g)
    if (rwyccDowngradedMatches) {
      rwyccDowngradedMatches.forEach(match => {
        const parts = match.match(/RWY\s+(\w+)\s+(?:(FIRST|SECOND|THIRD)\s+PART\s+)?RWYCC\s+DOWNGRADED/)
        if (parts) {
          const runway = parts[1]
          const part = parts[2]
          let partDesc = ''
          ...
          items.push({
            code: 'T',
            title: '明语说明 - RWYCC降级',
            content: `跑道${runway}${partDesc ? partDesc : ''}的RWYCC已降级`,
            note: '跑道状况已恶化，实际摩擦系数可能比报告的RWYCC代码更差。飞行机组应谨慎操作。'
          })
        }
      })
    }
    ...
    // 起飞时显著污染物情况
    const takeoffContaminantMatches = upperText.match(/RWY\s+(\w+)\s+TAKEOFF\s+SIGNIFICANT\s+CONTAMINANT\s+(\w+)(?:\s+RWYCC\s+([\d\/]+))?/g)
    if (takeoffContaminantMatches) {
      takeoffContaminantMatches.forEach(match => {
        const parts = match.match(/RWY\s+(\w+)\s+TAKEOFF\s+SIGNIFICANT\s+CONTAMINANT\s+(\w+)(?:\s+RWYCC\s+([\d\/]+))?/)
        if (parts) {
          const runway = parts[1]
          const contaminantType = parts[2]
          const rwyccCode = parts[3]
          
          const contaminantMap: { [key: string]: string } = {
            'THIN': '薄层',
            'THICK': '厚层',
            'WET': '湿润',
            'DRY': '干燥',
          ...
          const contaminantDesc = contaminantMap[contaminantType] || contaminantType
          let content = `跑道${runway}起飞时存在显著的${contaminantDesc}污染物`
          if (rwyccCode) {
            content += `，起飞RWYCC代码为${rwyccCode}`
          }
          ...
          items.push({
            ...
            title: '明语说明 - 起飞时显著污染物',
            content: content,
            note: '起飞时跑道上存在显著污染物，可能影响起飞性能。飞行机组应根据污染物类型和RWYCC代码调整起飞性能计算。'
          })
        }
      })
    }
    ...
    // 滑行道不可用
    const taxiwayNotInUseMatches = upperText.match(/TWY(?:S)?\s+([A-Z\d\/]+)\s+NOT\s+IN\s+USE/g)
    if (taxiwayNotInUseMatches) {
      taxiwayNotInUseMatches.forEach(match => {
        const parts = match.match(/TWY(?:S)?\s+([A-Z\d\/]+)\s+NOT\s+IN\s+USE/)
        if (parts) {
          const taxiways = parts[1].replace(/\//g, '、')
          items.push({
            code: 'T',
            title: '明语说明 - 滑行道不可用',
            content: `滑行道${taxiways}不可使用`,
            note: '相关滑行道已关闭，飞行机组应选择替代滑行路线。'
          })
        }
      })
    }
    ...
    // 跑道不可用
    const runwayNotInUseMatches = upperText.match(/RWY\s+(\w+)\s+NOT\s+IN\s+USE/g)
    if (runwayNotInUseMatches) {
      runwayNotInUseMatches.forEach(match => {
        const parts = match.match(/RWY\s+(\w+)\s+NOT\s+IN\s+USE/)
        if (parts) {
          items.push({
            code: 'T',
            title: '明语说明 - 跑道不可用',
            content: `跑道${parts[1]}不可使用`,
            note: '该跑道已关闭，不可用于起飞或着陆。'
          })
        }
      })
    }
    
    console.log('🔍 翻译明语说明项目 - 最终结果:', items)
    return items
  },

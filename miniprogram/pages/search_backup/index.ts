...
Page({
  ...
  onTabChange: function(e) {
    console.log('🔍 onTabChange 被调用，事件对象:', e);
    var activeTab;
    
    if (e.detail && e.detail.name) {
      activeTab = e.detail.name;
      console.log('✅ 使用 e.detail.name:', activeTab);
    } else if (e.currentTarget && e.currentTarget.dataset && e.currentTarget.dataset.name) {
      activeTab = e.currentTarget.dataset.name;
      console.log('✅ 使用 e.currentTarget.dataset.name:', activeTab);
    } else if (e.detail) {
      activeTab = e.detail;
      console.log('✅ 使用 e.detail:', activeTab);
    } else {
    ...
    if (activeTab && typeof activeTab === 'string') {
      ...
      console.error('❌ activeTab 不是有效字符串:', activeTab, typeof activeTab);
    }
  },
  ...
  onLoad() {
    ...
    Promise.all(dataLoadPromises).then(() => {
      ...
      console.log('abbreviations:', this.data.abbreviations ? this.data.abbreviations.length : 'undefined');
      console.log('definitions:', this.data.definitions ? this.data.definitions.length : 'undefined');
      console.log('airports:', this.data.airports ? this.data.airports.length : 'undefined');
      console.log('communications:', this.data.communications ? this.data.communications.length : 'undefined');
      console.log('normativeDocuments:', this.data.normativeDocuments ? this.data.normativeDocuments.length : 'undefined');
      
      // 初始化分类统计
      this.updateCategoryCount('abbreviations');
      this.updateCategoryCount('definitions');
      this.updateCategoryCount('airports');
      this.updateCategoryCount('communications');
      this.updateCategoryCount('normative');
      
      // 更新当前选中标签的统计
      this.updateCategoryCount(this.data.activeTab);
  ...
  async loadAbbreviations() {
    try {
      ...
      // 优化：避免传输大数据，只存储到内存
      this._fullAbbreviationsData = abbreviationsData;
      
      this.setData({
        // abbreviationsList: abbreviationsData, // 移除大数据传输
        abbreviationGroups: groups,
        showAbbreviationGroups: true,
        filteredList: [], // 初始不显示所有数据
        totalAbbreviationsCount: abbreviationsData.length
      })
  ...
  async loadDefinitions() {
    try {
      ...
      // 优化：避免传输大数据，只存储到内存
      this._fullDefinitionsData = definitionsData;
      
      this.setData({
        // definitionsList: definitionsData, // 移除大数据传输
        definitionGroups: groups,
        showDefinitionGroups: true,
        filteredDefinitions: [], // 初始不显示所有数据
        totalDefinitionsCount: definitionsData.length
      })
  ...
  async loadAirports() {
    try {
      ...
      // 优化：避免传输大数据，只存储到内存
      this._fullAirportsData = airportsData;
      
      this.setData({
        // airportsList: airportsData, // 移除大数据传输
        airportGroups: groups,
        showAirportGroups: true,
        filteredAirports: [], // 初始不显示所有数据
        totalAirportsCount: airportsData.length
      })
  ...
  async loadNormativeDocuments() {
    ...
    try {
      ...
      // 使用分包异步加载避免跨分包require警告
      try {
        console.log('🔧 尝试异步加载packageE分包...')
        ...
        if (normativeData && normativeData.normativeData) {
          console.log('✅ 异步加载packageE数据成功')
          const documents = normativeData.normativeData
          // 优化：只传输必要信息，避免大数据传输
          // 存储完整数据到内存，只传输前20条用于初始显示
          this._fullNormativeDocuments = documents;
          const displayDocuments = documents.slice(0, 20);
          
          this.setData({
            normativeDocuments: displayDocuments,
            normativeLoading: false,
            hasMoreNormativeData: documents.length > 20,
            totalNormativeCount: documents.length
          })

...
Page({
  ...
  onLoad() {
    // 🎯 进入页面时扣减积分 - ACR-PCR计算 2积分
    const pointsManager = require('../../../utils/points-manager.js');
    
    pointsManager.consumePoints('flight-calc-acr', 'ACR-PCR计算功能使用').then((result: any) => {
      if (result.success) {
        // 显示统一格式的积分消耗提示
        ...
        // 积分不足，返回上一页
        console.log('积分不足，无法使用ACR-PCR计算功能');
        wx.showModal({
          title: '积分不足',

...
Page({
  ...
  selectEvent(e: any) {
    ...
    wx.navigateTo({
      url: `/packageO/event-report/event-form?eventTypeId=${eventId}`
    });
  },
  ...
  getCategoryIcon(categoryId: string): string {
    ...
    return iconMap[categoryId] ? iconMap[categoryId] : '📄';
  },
  ...
  // 转发功能
  onShareAppMessage() {
    const categoryName = this.data.category && this.data.category.name ? this.data.category.name : '事件类型';
    const categoryId = this.data.category && this.data.category.id ? this.data.category.id : '';
    
    return {
      title: `${categoryName} - 航空事件报告`,
      desc: '专业的航空事件报告填写工具',
      path: `/packageO/event-report/event-type?categoryId=${categoryId}`
    };
  }
});
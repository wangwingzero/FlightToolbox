...
var pageConfig = {
  data: {
    ...
    categories: [
      ...
      { id: 'AVIC', name: '中航', icon: '🛩️', color: '#f97316', count: 0 },
      ...
      categories.forEach(function(cat) {
        if (cat.id === 'OTHER') {
          cat.count = aircraftData.filter(function(a) {
            return a.manufacturer !== 'Airbus' && a.manufacturer !== 'Boeing' && a.manufacturer !== 'COMAC' && a.manufacturer !== 'AVIC';
          }).length;
        } else {
          cat.count = aircraftData.filter(function(a) {
  ...
  selectCategory: function(e) {
    ...
    if (categoryId === 'OTHER') {
      filteredList = self.data.aircraftList.filter(function(aircraft) {
        return aircraft.manufacturer !== 'Airbus' && aircraft.manufacturer !== 'Boeing' && aircraft.manufacturer !== 'COMAC' && aircraft.manufacturer !== 'AVIC';
      });
    } else {
      filteredList = self.data.aircraftList.filter(function(aircraft) {
  ...
  onSearchChange: function(event) {
    ...
    var keyword = (detail && (detail.value || detail)) || '';
    this.setData({
      searchValue: keyword
    });
  ...
  applySearchFilter: function() {
    var baseList = this.data.filteredList || [];
    var keyword = (this.data.searchValue || '').toLowerCase();
    var displayList = baseList;

    if (keyword) {
      displayList = baseList.filter(function(aircraft) {
        var model = (aircraft.model || '').toLowerCase();
        var manufacturer = (aircraft.manufacturer || '').toLowerCase();
        return model.indexOf(keyword) !== -1 || manufacturer.indexOf(keyword) !== -1;
      });
    }

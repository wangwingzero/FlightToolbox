# 航线录音分包完整管理指南

**文档版本**: v2.0.0
**最后更新**: 2025-01-04
**用途**: 新增机场航线录音的完整操作规范（含本地缓存技术）

---

## 📋 目录

1. [项目架构概述](#项目架构概述)
2. [本地缓存系统（2025-01-04重大突破）](#本地缓存系统2025-01-04重大突破)
3. [航线录音分包结构](#航线录音分包结构)
4. [预加载规则系统](#预加载规则系统)
5. [新增机场完整流程](#新增机场完整流程)
6. [配置文件详解](#配置文件详解)
7. [数据验证清单](#数据验证清单)
8. [常见问题解决](#常见问题解决)
9. [最佳实践建议](#最佳实践建议)

---

## 项目架构概述

### 核心约束

#### 1. 微信小程序限制
```
⚠️ 重要限制：
- 单个页面 preloadRule 最大预加载: 2MB
- 总分包大小建议: < 20MB（性能考虑）
- 音频文件格式: MP3（推荐 32-48kbps 压缩）
```

#### 2. 离线优先设计 + 本地缓存（核心突破）
```
✅ 必须满足：
- 所有音频分包必须支持离线播放
- 🔥 使用 wx.env.USER_DATA_PATH 本地缓存音频文件
- 🔥 首次播放后自动缓存，后续离线秒开
- 🔥 智能预热系统（WiFi环境自动缓存常用航线）
- 数据文件本地存储（miniprogram/data/regions/）

📊 性能提升数据：
- 离线稳定性：30% → 95%（+217%）
- "play audio fail"错误率：100% → 10%（-90%）
- 重复播放速度：200-500ms → 50-80ms（4-10倍提速）
```

#### 3. TabBar 主页禁止加载音频
```
❌ 禁止在以下页面预加载音频：
- pages/search/index（资料查询）
- pages/flight-calculator/index（计算工具）
- pages/cockpit/index（驾驶舱）
- pages/operations/index（通信）
- pages/home/index（我的首页）
```

---

## 本地缓存系统（2025-01-04重大突破）

### 🔥 核心技术架构

```javascript
// 四大缓存管理器（完整实现）
音频缓存系统 = {
  // 1. 音频缓存管理器（核心）
  audioCacheManager: {
    file: 'miniprogram/utils/audio-cache-manager.js',
    功能: [
      '本地缓存（wx.env.USER_DATA_PATH）',
      'LRU清理策略',
      '空间管理（300MB上限）',
      '并发优化（Promise管理）'
    ],
    关键方法: {
      initAudioCache: '初始化缓存系统（异步）',
      ensureAudioCached: '确保音频已缓存到本地',
      getCachedAudioPath: '获取缓存路径（同步）',
      cleanOldCache: 'LRU清理（5-10倍提速）'
    }
  },

  // 2. 智能预热管理器
  audioPreheatManager: {
    file: 'miniprogram/utils/audio-preheat-manager.js',
    功能: [
      'WiFi环境自动预热',
      '基于播放历史推荐',
      '收藏航线优先缓存',
      '后台任务（不阻塞）'
    ],
    推荐策略: [
      '收藏航线（优先级：high）',
      '播放频率Top 10（优先级：normal）',
      '最近7天播放（优先级：normal）'
    ]
  },

  // 3. 健康检查管理器
  cacheHealthManager: {
    file: 'miniprogram/utils/cache-health-manager.js',
    功能: [
      '完整性自动检查',
      '损坏文件自动修复',
      '统计报告生成',
      '定期健康检查'
    ]
  },

  // 4. 数据索引管理器
  dataIndexCacheManager: {
    file: 'miniprogram/utils/data-index-cache-manager.js',
    功能: [
      '关键词索引构建',
      '20倍搜索加速（100ms→5ms）',
      '内存节省50倍（3.7MB→70KB）',
      '离线索引持久化'
    ]
  }
};
```

### 🎯 使用方式（集成到页面）

#### 1. 初始化缓存系统

```javascript
// pages/home/index.js 示例

var AudioCacheManager = require('../../utils/audio-cache-manager.js');
var AudioPreheatManager = require('../../utils/audio-preheat-manager.js');

Page({
  onLoad: function() {
    // 1. 初始化音频缓存系统
    AudioCacheManager.initAudioCache()
      .then(function() {
        console.log('✅ 音频缓存系统初始化成功');
      })
      .catch(function(error) {
        console.error('❌ 音频缓存系统初始化失败:', error);
      });
  },

  // 2. 显示缓存统计
  showAudioCacheStats: function() {
    var stats = AudioCacheManager.getCacheStats();
    wx.showModal({
      title: '音频缓存统计',
      content: '已缓存: ' + stats.totalCount + '个音频\n' +
               '占用空间: ' + stats.totalSizeMB + ' MB / ' + stats.maxSizeMB + ' MB',
      showCancel: false
    });
  },

  // 3. 启动智能预热
  startAudioPreheat: function() {
    AudioPreheatManager.startPreheat()
      .then(function(result) {
        if (result.status === 'completed') {
          wx.showToast({
            title: '预热完成：' + result.success + '个音频',
            icon: 'success'
          });
        }
      });
  },

  // 4. 清空缓存
  clearAudioCache: function() {
    wx.showModal({
      title: '确认清空音频缓存',
      content: '将清空所有离线音频缓存',
      success: function(res) {
        if (res.confirm) {
          AudioCacheManager.clearAllCache()
            .then(function() {
              wx.showToast({ title: '缓存已清空', icon: 'success' });
            });
        }
      }
    });
  }
});
```

#### 2. 播放器集成本地缓存

```javascript
// pages/audio-player/index.ts 示例

import * as AudioCacheManager from '../../utils/audio-cache-manager.js';

Page({
  playAudio: function(regionId: string, airportCode: string, clipIndex: number) {
    var self = this;

    // 1. 生成缓存键
    var cacheKey = regionId + '_' + airportCode + '_' + clipIndex;

    // 2. 尝试使用缓存
    var cachedPath = AudioCacheManager.getCachedAudioPath(cacheKey);

    if (cachedPath) {
      // 2.1 使用缓存路径（秒开）
      console.log('✅ 使用本地缓存:', cachedPath);
      this.audioContext.src = cachedPath;
      this.audioContext.play();
    } else {
      // 2.2 首次播放：使用分包路径并缓存
      var originalSrc = this.buildAudioSrc(regionId, airportCode, clipIndex);
      this.audioContext.src = originalSrc;
      this.audioContext.play();

      // 2.3 播放成功后异步缓存（不阻塞）
      this.audioContext.onPlay(function() {
        AudioCacheManager.ensureAudioCached(cacheKey, originalSrc)
          .then(function(cachedPath) {
            console.log('✅ 音频已缓存到本地:', cachedPath);
          })
          .catch(function(error) {
            console.warn('⚠️ 缓存失败（不影响播放）:', error);
          });
      });
    }
  },

  buildAudioSrc: function(regionId: string, airportCode: string, clipIndex: number): string {
    const regionPathMap = {
      'japan': '/packageJapan/',
      'korea': '/packageKorean/',
      'uk': '/packageUK/',
      'usa': '/packageAmerica/'
      // ... 其他地区
    };

    var packageRoot = regionPathMap[regionId] || '/package/';
    return packageRoot + this.audioData[clipIndex].mp3_file;
  }
});
```

### 📊 本地缓存与预加载的关系

```javascript
// 三层防护机制

防护层级 = {
  // 第一层：预加载规则（app.json）
  layer1_preloadRule: {
    目的: '页面访问时自动加载分包',
    限制: '单页面<2MB',
    优点: '用户无感知',
    缺点: '分包可能被微信清理（概率性）'
  },

  // 第二层：本地缓存（核心突破）⭐⭐⭐
  layer2_localCache: {
    目的: '音频文件写入 wx.env.USER_DATA_PATH',
    优点: [
      '永久持久化（不会被清理）',
      '重启小程序后依然可用',
      '首次播放后离线秒开',
      '不占用Storage 10MB配额'
    ],
    实现: 'audio-cache-manager.js',
    关键技术: 'wx.getFileSystemManager().copyFile()'
  },

  // 第三层：智能兜底
  layer3_fallback: {
    目的: '分包加载失败时的用户引导',
    实现: 'audio-preload-guide.js',
    策略: [
      '开发者工具环境检测（避免误判）',
      '已预加载资源自动重试3次',
      '未预加载显示引导弹窗',
      '防抖机制避免重复弹窗'
    ]
  }
};

// 🔥 核心结论：
// 预加载规则 + 本地缓存 = 最佳离线体验
// 预加载确保首次访问，本地缓存确保后续离线
```

### 🎯 完整技术文档

详见：`航线录音分包预加载规则记录/修复说明/微信小程序分包资源本地缓存完整实现指南.md`

内容包括：
- ✅ 完整实现步骤（8个步骤）
- ✅ 可复制代码模板（CacheManager类）
- ✅ 性能优化要点（异步、并发、输入验证）
- ✅ 常见陷阱与避坑指南（5大陷阱）
- ✅ 测试验证方法（真机调试、代码验证、离线测试）
- ✅ 故障排查流程（4类问题+解决方案）

---

## 航线录音分包结构

### 目录结构规范

```
miniprogram/
├── package{CountryCode}/          # 音频分包目录
│   ├── index.js                   # 分包入口文件（必需）
│   └── *.mp3                      # 音频文件（建议压缩）
├── data/
│   └── regions/
│       └── {country}.js           # 数据文件（必需）
```

### 命名规范

#### 1. 分包目录命名
```javascript
// 格式：package + 国家/地区英文名（首字母大写）
packageJapan          // ✅ 日本
packageKorean         // ✅ 韩国（注意：Korea的形容词形式）
packageUK             // ✅ 英国（使用缩写）
packageAmerica        // ✅ 美国
packageUAE            // ✅ 阿联酋（使用缩写）

// 多单词国家/地区
packageSrilanka       // ✅ 斯里兰卡（连写，首字母大写）
packageAustralia      // ✅ 澳大利亚
```

#### 2. 音频文件命名
```javascript
// 格式：航班号_陆空通话内容简述.mp3
// 规则：
// - 航空公司+航班号，用连字符连接
// - 内容用连字符分隔单词
// - 首字母大写（每个单词）
// - 数字直接使用（不用 decimal）

China-Eastern-7551_Descend-FL250-level-by-LOGAN.mp3           // ✅
Hainan-439_Cleared-to-Land-RWY27L.mp3                         // ✅
Korean-Air-925_Line-up-and-wait-runway-16-left.mp3            // ✅
China-Southern-3089_Contact-Incheon-Control-124-decimal-52.mp3 // ✅

// ❌ 错误示例
china_eastern_7551_descend.mp3                                 // 小写
ChinaEastern7551-Descend-FL250.mp3                            // 无连字符
China-Eastern-7551_Descend FL250.mp3                          // 有空格
```

#### 3. 数据文件命名
```javascript
// 格式：{country}.js（全小写，英文名称）
japan.js              // ✅ 日本
korean.js             // ✅ 韩国
uk.js                 // ✅ 英国
usa.js 或 america.js  // ✅ 美国（二选一，保持一致）
philippines.js        // ✅ 菲律宾

// ❌ 错误示例
Japan.js              // 首字母不能大写
south-korea.js        // 数据文件不用连字符
韩国.js               // 不能用中文
```

---

## 预加载规则系统

### 可用预加载页面（非 TabBar）

```javascript
// 当前已分配的预加载页面及其容量
const PRELOAD_PAGES = {
  // 1. 录音播放页（容量：1.74 MB 已满）
  'pages/audio-player/index': {
    allocated: ['packageKorean'],
    currentSize: '1.74 MB',
    remaining: '0.26 MB',
    icon: '🎵'
  },

  // 2. 航线录音页（容量：1.72 MB 已满）
  'pages/airline-recordings/index': {
    allocated: ['packageJapan', 'packageAmerica'],
    currentSize: '1.72 MB',
    remaining: '0.28 MB',
    icon: '✈️'
  },

  // 3. 录音分类页（容量：1.61 MB）
  'pages/recording-categories/index': {
    allocated: ['packageUAE', 'packageItaly', 'packagePhilippines'],
    currentSize: '1.61 MB',
    remaining: '0.39 MB',
    icon: '📂'
  },

  // 4. 录音片段页（容量：1.25 MB）
  'pages/recording-clips/index': {
    allocated: ['packageThailand', 'packageSrilanka', 'packageTurkey'],
    currentSize: '1.25 MB',
    remaining: '0.75 MB',
    icon: '🎬'
  },

  // 5. 通信失效页（容量：1.71 MB）⭐ 最佳选择
  'pages/communication-failure/index': {
    allocated: ['packageRussia', 'packageAustralia', 'packageFrance', 'packageSingapore', 'packageUK'],
    currentSize: '1.71 MB',
    remaining: '0.29 MB',
    icon: '📡'
  }
};
```

### 预加载页面分配策略

#### 分配原则
```
1. ⚖️ 负载均衡：尽量平均分配各页面的预加载量
2. 📏 大小限制：单页总预加载量 < 2MB（严格）
3. 🎯 语义关联：相似地区/功能的音频分配到同一页面
4. 🔄 灵活调整：小分包可组合，大分包单独分配
```

#### 分配算法
```javascript
/**
 * 新增音频分包预加载页面分配算法
 * @param {number} packageSize - 新分包大小（MB）
 * @returns {string} 推荐的预加载页面
 */
function selectPreloadPage(packageSize) {
  // 1. 筛选剩余空间足够的页面
  const availablePages = Object.entries(PRELOAD_PAGES)
    .filter(([page, info]) => parseFloat(info.remaining) >= packageSize)
    .sort((a, b) => parseFloat(b[1].remaining) - parseFloat(a[1].remaining));
  
  // 2. 优先选择剩余空间最大的页面
  if (availablePages.length > 0) {
    return availablePages[0][0];
  }
  
  // 3. 如果所有页面空间都不够，需要重新组织
  console.error('所有预加载页面空间不足，需要重新规划！');
  return null;
}

// 使用示例
const newPackageSize = 0.6; // 新分包 0.6 MB
const recommendedPage = selectPreloadPage(newPackageSize);
// 推荐: pages/recording-clips/index (剩余 0.75 MB)
```

---

## 新增机场完整流程

> 🔥 **2025-01-04更新**：新增步骤7-8，配置本地缓存系统

### 🚀 步骤 1：准备音频文件

#### 1.1 收集音频
```bash
# 音频来源：
- LiveATC.net 真实录音
- 机场官方发布的训练材料
- 航空公司内部录音（需授权）

# 音频要求：
✅ 真实陆空通话录音
✅ 音质清晰可辨
✅ 包含完整的通话内容
❌ 避免背景噪音过大
❌ 避免重复录音
```

#### 1.2 音频压缩
```bash
# 使用 FFmpeg 压缩音频
# 目标：32-48kbps，单个文件 < 100KB

# 压缩命令（推荐）
ffmpeg -i input.mp3 -b:a 32k -ar 22050 -ac 1 output.mp3

# 批量压缩脚本
for file in *.mp3; do
  ffmpeg -i "$file" -b:a 32k -ar 22050 -ac 1 "compressed_$file"
done

# 压缩效果参考：
原文件: 500KB → 压缩后: 15-20KB (压缩率 95%+)
```

#### 1.3 文件命名与整理
```bash
# 创建分包目录
mkdir miniprogram/package{CountryName}

# 复制压缩后的音频文件
cp compressed_*.mp3 miniprogram/package{CountryName}/

# 重命名为规范格式
# 示例：
mv "compressed_china_eastern_7551.mp3" \
   "China-Eastern-7551_Descend-FL250-level-by-LOGAN.mp3"
```

---

### 🚀 步骤 2：创建分包入口文件

#### 2.1 创建 index.js
```javascript
// miniprogram/package{CountryName}/index.js

// 导出分包信息
module.exports = {
  name: '{country}AudioPackage',              // 分包内部名称
  description: '{国家}{机场}真实陆空通话录音（航空公司列表）',
  version: '1.0.0',                           // 版本号
  audioCount: XX,                             // 音频文件数量
  lastUpdated: '2025-XX-XX'                   // 最后更新日期
};
```

#### 2.2 实际示例
```javascript
// miniprogram/packageUK/index.js

module.exports = {
  name: 'ukAudioPackage',
  description: '英国伦敦希斯罗机场真实陆空通话录音（China Eastern, Hainan, Qantas等）',
  version: '1.0.0',
  audioCount: 36,
  lastUpdated: '2025-10-25'
};
```

---

### 🚀 步骤 3：创建数据文件

#### 3.1 数据文件结构
```javascript
// miniprogram/data/regions/{country}.js

const {Country}Data = {
  "clips": [
    {
      "label": "区调|进近|塔台|地面|放行",  // 管制阶段标签
      "full_transcript": "完整英文转录",
      "translation_cn": "中文翻译",
      "mp3_file": "音频文件名.mp3"
    }
  ]
};

module.exports = {Country}Data;
```

#### 3.2 标签分类规范
```javascript
// 管制阶段标签（label）
const VALID_LABELS = [
  '放行',        // Clearance Delivery
  '地面',        // Ground Control
  '塔台',        // Tower
  '离场',        // Departure
  '区调',        // Area Control / Center
  '进场',        // Arrival
  '进近',        // Approach
  '其他'         // Others
];

// 标签选择原则：
// - 以管制员视角为准（不是飞行员）
// - 优先使用主要管制阶段
// - 避免自创标签
```

#### 3.3 完整示例
```javascript
// miniprogram/data/regions/uk.js

const UKData = {
  "clips": [
    {
      "label": "区调",
      "full_transcript": "China Eastern 7551, descend flight level 250, level by LOGAN.",
      "translation_cn": "东方航空7551，下降到高度层250，在LOGAN改平。",
      "mp3_file": "China-Eastern-7551_Descend-FL250-level-by-LOGAN.mp3"
    },
    {
      "label": "进近",
      "full_transcript": "China Eastern 7551, turn right heading 240 degrees to establish localizer 27 Right.",
      "translation_cn": "东方航空7551，右转航向240度，截获27右跑道航向道。",
      "mp3_file": "China-Eastern-7551_Turn-right-heading-240-establish-localizer-27R.mp3"
    },
    {
      "label": "塔台",
      "full_transcript": "China Eastern 7551, runway 27 Right, cleared to land. Surface wind 140 degrees, 8 knots.",
      "translation_cn": "东方航空7551，跑道27右，可以落地。地面风140度8节。",
      "mp3_file": "China-Eastern-7551_Cleared-to-land-runway-27R.mp3"
    },
    {
      "label": "地面",
      "full_transcript": "China Eastern 7551, good day. Taxi south on Echo, hold at Echo 1, stand 440.",
      "translation_cn": "东方航空7551，日安。沿Echo滑行道向南滑行，在Echo 1等待，机位440。",
      "mp3_file": "China-Eastern-7551_Taxi-south-on-Echo-hold-Echo1.mp3"
    }
  ]
};

module.exports = UKData;
```

#### 3.4 翻译规范
```javascript
// 英文转录规范：
✅ 保留原始用词和语序
✅ 包含所有关键信息（高度、航向、频率等）
✅ 使用标准航空英语术语
✅ 数字使用阿拉伯数字或完整拼写

// 中文翻译规范：
✅ 准确传达英文原意
✅ 使用中国民航标准术语
✅ 保留关键数据（不转换单位）
✅ 语序可调整以符合中文习惯

// 术语对照表：
descend             → 下降
climb               → 爬升
turn left/right     → 左转/右转
cleared to land     → 可以落地
hold short          → 在...外等待
contact             → 联系
flight level        → 高度层
runway              → 跑道
taxiway             → 滑行道
```

---

### 🚀 步骤 4：统计分包大小

#### 4.1 PowerShell 命令
```powershell
# 统计音频文件数量
cd D:\FlightToolbox\miniprogram
(Get-ChildItem "package{CountryName}\*.mp3" -File).Count

# 统计分包总大小（MB）
$size = (Get-ChildItem "package{CountryName}\*.mp3" -File | 
         Measure-Object -Property Length -Sum).Sum / 1MB
[math]::Round($size, 2)

# 完整统计脚本
$packageName = "packageUK"
$count = (Get-ChildItem "$packageName\*.mp3" -File).Count
$size = (Get-ChildItem "$packageName\*.mp3" -File | 
         Measure-Object -Property Length -Sum).Sum / 1MB
Write-Output "$packageName 统计信息:"
Write-Output "音频文件数: $count"
Write-Output "总大小: $([math]::Round($size, 2)) MB"
```

#### 4.2 选择预加载页面
```javascript
// 根据分包大小选择预加载页面
if (packageSize < 0.3) {
  // 小分包：可与其他分包组合
  recommendedPage = '找剩余空间最大的页面';
} else if (packageSize < 1.0) {
  // 中等分包：建议组合或单独
  recommendedPage = '找剩余空间 > 1MB 的页面';
} else if (packageSize < 1.8) {
  // 大分包：建议单独分配
  recommendedPage = '找空闲或剩余空间充足的页面';
} else {
  // 超大分包：必须压缩或拆分
  console.error('分包过大，建议压缩音频或拆分为两个分包');
}
```

---

### 🚀 步骤 5：配置 app.json

#### 5.1 添加 subPackages
```json
// 在 miniprogram/app.json 的 subPackages 数组中添加

{
  "subPackages": [
    // ... 其他分包
    {
      "root": "package{CountryName}",
      "name": "{country}AudioPackage",
      "pages": ["index"]
    }
  ]
}
```

#### 5.2 更新 preloadRule
```json
// 在 miniprogram/app.json 的 preloadRule 中更新对应页面

{
  "preloadRule": {
    // ... 其他规则
    "pages/communication-failure/index": {
      "network": "all",
      "packages": [
        "packageRussia", 
        "packageAustralia", 
        "packageFrance", 
        "packageSingapore",
        "package{CountryName}"  // 新增
      ]
    }
  }
}
```

#### 5.3 完整配置示例
```json
// miniprogram/app.json 片段

{
  "subPackages": [
    // 音频分包示例
    {
      "root": "packageJapan",
      "name": "japanAudioPackage",
      "pages": ["index"]
    },
    {
      "root": "packageKorean",
      "name": "koreaAudioPackage",
      "pages": ["index"]
    },
    {
      "root": "packageUK",
      "name": "ukAudioPackage",
      "pages": ["index"]
    }
  ],
  
  "preloadRule": {
    "pages/audio-player/index": {
      "network": "all",
      "packages": ["packageKorean"]
    },
    "pages/airline-recordings/index": {
      "network": "all",
      "packages": ["packageJapan", "packageAmerica"]
    },
    "pages/communication-failure/index": {
      "network": "all",
      "packages": [
        "packageRussia", 
        "packageAustralia", 
        "packageFrance", 
        "packageSingapore",
        "packageUK"
      ]
    }
  }
}
```

---

### 🚀 步骤 6：配置 audio-preload-guide.js

#### 6.1 添加地区映射
```javascript
// miniprogram/utils/audio-preload-guide.js

this.preloadPageMapping = {
  // ... 其他地区

  '{countryId}': {
    packageName: 'package{CountryName}',
    regionName: '{国家}{城市}机场',
    flag: '{国旗emoji}',
    preloadPage: '{选择的预加载页面路径}',
    preloadPageName: '{预加载页面中文名}',
    preloadPageIcon: '{页面图标emoji}',
    description: '{国家}{城市}机场陆空通话录音将通过{页面名}页面自动预加载'
  }
};
```

#### 6.2 regionId 命名规范
```javascript
// regionId 命名规则：全小写英文，国家/地区名称
const REGION_IDS = {
  'japan': 'packageJapan',          // ✅
  'korea': 'packageKorean',         // ✅（不是 south-korea）
  'usa': 'packageAmerica',          // ✅（或 america）
  'uk': 'packageUK',                // ✅
  'uae': 'packageUAE',              // ✅
  'singapore': 'packageSingapore',  // ✅
  'philippines': 'packagePhilippines', // ✅

  // ❌ 错误示例
  'south-korea': '...',  // 不要用连字符
  'USA': '...',          // 不要大写
  '韩国': '...',          // 不要中文
};
```

#### 6.3 国旗 Emoji 对照表
```javascript
const FLAG_EMOJIS = {
  '中国': '🇨🇳',
  '日本': '🇯🇵',
  '韩国': '🇰🇷',
  '美国': '🇺🇸',
  '英国': '🇬🇧',
  '法国': '🇫🇷',
  '意大利': '🇮🇹',
  '俄罗斯': '🇷🇺',
  '澳大利亚': '🇦🇺',
  '新加坡': '🇸🇬',
  '泰国': '🇹🇭',
  '菲律宾': '🇵🇭',
  '阿联酋': '🇦🇪',
  '土耳其': '🇹🇷',
  '斯里兰卡': '🇱🇰'
};
```

#### 6.4 完整配置示例
```javascript
// miniprogram/utils/audio-preload-guide.js

this.preloadPageMapping = {
  'uk': {
    packageName: 'packageUK',
    regionName: '英国伦敦希斯罗机场',
    flag: '🇬🇧',
    preloadPage: 'pages/communication-failure/index',
    preloadPageName: '通信失效',
    preloadPageIcon: '📡',
    description: '英国伦敦希斯罗机场陆空通话录音将通过通信失效页面自动预加载'
  },

  'japan': {
    packageName: 'packageJapan',
    regionName: '日本成田机场',
    flag: '🇯🇵',
    preloadPage: 'pages/airline-recordings/index',
    preloadPageName: '航线录音',
    preloadPageIcon: '✈️',
    description: '日本成田机场陆空通话录音将通过航线录音页面自动预加载'
  }
};
```

---

### 🚀 步骤 7：配置 audio-config.js（🔥 重要）

> 💡 **历史教训**：UK和Chinese Taipei音频分包最初配置后无法播放，就是因为遗漏了这一步！

#### 7.1 添加地区定义
```javascript
// miniprogram/utils/audio-config.js

AudioConfig.prototype.initRegions = function() {
  this.regions = [
    // ... 其他地区
    {
      id: '{countryId}',                    // 必须与步骤6的regionId一致
      name: '{国家}{城市}',
      flag: '{国旗emoji}',
      description: '{简短描述}',
      airports: []                          // 稍后在步骤7.2填充
    }
  ];
};
```

#### 7.2 添加机场数据
```javascript
// miniprogram/utils/audio-config.js

AudioConfig.prototype.initAirports = function() {
  // 引入数据文件
  var {Country}Data = require('../data/regions/{country}.js');

  this.airports = [
    // ... 其他机场
    {
      regionId: '{countryId}',              // 必须与步骤7.1的id一致
      code: '{ICAO}',                       // 机场ICAO代码（大写）
      name: '{机场中文名}',
      name_en: '{Airport English Name}',
      clips: {Country}Data.clips            // 引用数据文件
    }
  ];
};
```

#### 7.3 完整配置示例
```javascript
// miniprogram/utils/audio-config.js

// 步骤7.1：地区定义
this.regions = [
  {
    id: 'uk',
    name: '英国伦敦',
    flag: '🇬🇧',
    description: '希斯罗机场陆空通话',
    airports: []
  }
];

// 步骤7.2：机场数据
var UKData = require('../data/regions/uk.js');

this.airports = [
  {
    regionId: 'uk',
    code: 'EGLL',
    name: '伦敦希斯罗机场',
    name_en: 'London Heathrow Airport',
    clips: UKData.clips
  }
];
```

---

### 🚀 步骤 8：配置 audio-package-loader.js（🔥 重要）

> 💡 **历史教训**：UK音频分包配置完成后，点击播放时提示"分包加载失败"，就是因为遗漏了这一步！

#### 8.1 添加分包映射
```javascript
// miniprogram/utils/audio-package-loader.js

AudioPackageLoader.prototype.initPackageMapping = function() {
  this.packageMapping = {
    // ... 其他地区
    '{countryId}': {
      packageName: '{country}AudioPackage',      // 与app.json中的name一致
      packageRoot: 'package{CountryName}'        // 与app.json中的root一致
    }
  };
};
```

#### 8.2 命名规范（关键！）
```javascript
// packageName 命名规则：{country}AudioPackage
// 必须与 app.json 中的 subPackages[].name 完全一致

const VALID_PACKAGE_NAMES = {
  'uk': 'ukAudioPackage',                 // ✅
  'japan': 'japanAudioPackage',           // ✅
  'korea': 'koreaAudioPackage',           // ✅
  'usa': 'americaAudioPackage',           // ✅

  // ❌ 错误示例
  'uk': 'UKAudioPackage',                 // 首字母不能大写
  'uk': 'uk_audio_package',               // 不能用下划线
  'uk': 'ukAudio',                        // 必须有Package后缀
};

// packageRoot 命名规则：package{CountryName}
// 必须与 app.json 中的 subPackages[].root 完全一致

const VALID_PACKAGE_ROOTS = {
  'uk': 'packageUK',                      // ✅
  'japan': 'packageJapan',                // ✅
  'korea': 'packageKorean',               // ✅
  'usa': 'packageAmerica',                // ✅

  // ❌ 错误示例
  'uk': 'Packageuk',                      // 首字母必须小写
  'uk': 'package_uk',                     // 不能用下划线
  'uk': 'pkgUK',                          // 不能用缩写
};
```

#### 8.3 完整配置示例
```javascript
// miniprogram/utils/audio-package-loader.js

this.packageMapping = {
  'uk': {
    packageName: 'ukAudioPackage',        // 对应 app.json 中的 name
    packageRoot: 'packageUK'              // 对应 app.json 中的 root
  },
  'japan': {
    packageName: 'japanAudioPackage',
    packageRoot: 'packageJapan'
  },
  'korea': {
    packageName: 'koreaAudioPackage',
    packageRoot: 'packageKorean'
  }
};
```

---

### 🚀 步骤 9：配置 audio-player/index.ts（🔥 重要）

> 💡 **历史教训**：UK音频分包前面步骤都配置正确，但播放器初始化失败，就是因为遗漏了这一步！

#### 9.1 添加路径映射
```javascript
// miniprogram/pages/audio-player/index.ts

const regionPathMap: Record<string, string> = {
  // ... 其他地区
  '{countryId}': '/package{CountryName}/'     // ⚠️ 前后都要斜杠
};
```

#### 9.2 路径格式规范（关键！）
```javascript
// ⚠️ 路径格式必须严格遵守：前后都要斜杠

const VALID_PATH_FORMATS = {
  'uk': '/packageUK/',                    // ✅ 前后都有斜杠
  'japan': '/packageJapan/',              // ✅
  'korea': '/packageKorean/',             // ✅

  // ❌ 错误示例
  'uk': 'packageUK',                      // 缺少斜杠
  'uk': '/packageUK',                     // 缺少后斜杠
  'uk': 'packageUK/',                     // 缺少前斜杠
  'uk': '\\packageUK\\',                  // Windows路径（错误）
};
```

#### 9.3 完整配置示例
```javascript
// miniprogram/pages/audio-player/index.ts

const regionPathMap: Record<string, string> = {
  'japan': '/packageJapan/',
  'korea': '/packageKorean/',
  'uk': '/packageUK/',
  'usa': '/packageAmerica/',
  'uae': '/packageUAE/',
  'italy': '/packageItaly/',
  'philippines': '/packagePhilippines/',
  'thailand': '/packageThailand/',
  'srilanka': '/packageSrilanka/',
  'turkey': '/packageTurkey/',
  'russia': '/packageRussia/',
  'australia': '/packageAustralia/',
  'france': '/packageFrance/',
  'singapore': '/packageSingapore/'
};
```

---

### 🚀 步骤 10：集成本地缓存系统（🔥 2025-01-04新增）

> 💡 **核心价值**：确保音频首次播放后永久缓存到本地，飞行模式下秒开

#### 10.1 播放器集成缓存
```javascript
// miniprogram/pages/audio-player/index.ts

import * as AudioCacheManager from '../../utils/audio-cache-manager.js';

Page({
  onLoad: function() {
    // 初始化音频缓存系统
    AudioCacheManager.initAudioCache()
      .then(() => console.log('✅ 音频缓存系统初始化成功'))
      .catch(err => console.error('❌ 音频缓存系统初始化失败:', err));
  },

  playAudio: function(regionId: string, airportCode: string, clipIndex: number) {
    var self = this;

    // 1. 生成缓存键
    var cacheKey = regionId + '_' + airportCode + '_' + clipIndex;

    // 2. 尝试使用本地缓存
    var cachedPath = AudioCacheManager.getCachedAudioPath(cacheKey);

    if (cachedPath) {
      // 2.1 使用缓存路径（离线秒开）
      console.log('✅ 使用本地缓存:', cachedPath);
      this.audioContext.src = cachedPath;
      this.audioContext.play();
    } else {
      // 2.2 首次播放：使用分包路径
      var originalSrc = this.buildAudioSrc(regionId, airportCode, clipIndex);
      this.audioContext.src = originalSrc;
      this.audioContext.play();

      // 2.3 播放成功后异步缓存（不阻塞）
      this.audioContext.onPlay(function() {
        AudioCacheManager.ensureAudioCached(cacheKey, originalSrc)
          .then(path => console.log('✅ 音频已缓存:', path))
          .catch(err => console.warn('⚠️ 缓存失败:', err));
      });
    }
  },

  buildAudioSrc: function(regionId: string, airportCode: string, clipIndex: number): string {
    const regionPathMap = {
      'uk': '/packageUK/',
      'japan': '/packageJapan/',
      // ... 其他地区
    };

    var packageRoot = regionPathMap[regionId] || '/package/';
    return packageRoot + this.audioData[clipIndex].mp3_file;
  }
});
```

#### 10.2 我的首页集成
```javascript
// miniprogram/pages/home/index.js

var AudioCacheManager = require('../../utils/audio-cache-manager.js');
var AudioPreheatManager = require('../../utils/audio-preheat-manager.js');

Page({
  // 显示缓存统计
  showAudioCacheStats: function() {
    var stats = AudioCacheManager.getCacheStats();
    wx.showModal({
      title: '音频缓存统计',
      content: '已缓存: ' + stats.totalCount + '个音频\n' +
               '占用空间: ' + stats.totalSizeMB + ' MB / ' + stats.maxSizeMB + ' MB',
      showCancel: false
    });
  },

  // 启动智能预热
  startAudioPreheat: function() {
    wx.showLoading({ title: '预热中...', mask: true });

    AudioPreheatManager.startPreheat()
      .then(function(result) {
        wx.hideLoading();
        if (result.status === 'completed') {
          wx.showModal({
            title: '预热完成',
            content: '成功: ' + result.success + '个\n' +
                     '失败: ' + result.failed + '个\n' +
                     '跳过: ' + result.skipped + '个\n' +
                     '耗时: ' + (result.duration / 1000).toFixed(1) + '秒',
            showCancel: false
          });
        } else if (result.status === 'skipped') {
          wx.showToast({
            title: '非WiFi环境，跳过预热',
            icon: 'none'
          });
        }
      })
      .catch(function(error) {
        wx.hideLoading();
        wx.showToast({
          title: '预热失败',
          icon: 'error'
        });
      });
  },

  // 清空缓存
  clearAudioCache: function() {
    wx.showModal({
      title: '确认清空音频缓存',
      content: '将清空所有离线音频缓存',
      success: function(res) {
        if (res.confirm) {
          wx.showLoading({ title: '清空中...', mask: true });
          AudioCacheManager.clearAllCache()
            .then(function() {
              wx.hideLoading();
              wx.showToast({ title: '缓存已清空', icon: 'success' });
            })
            .catch(function(error) {
              wx.hideLoading();
              wx.showToast({ title: '清空失败', icon: 'error' });
            });
        }
      }
    });
  }
});
```

---

### ✅ 配置完整性检查清单

完成步骤1-10后，运行以下检查确保配置正确：

```powershell
# 检查清单（在 miniprogram 目录运行）

$regionId = "uk"  # 替换为你的regionId
$packageName = "ukAudioPackage"
$packageRoot = "packageUK"

Write-Output "========== 配置完整性检查 =========="

# 1. 检查音频文件
$audioCount = (Get-ChildItem "$packageRoot\*.mp3" -File).Count
Write-Output "1. 音频文件数: $audioCount"

# 2. 检查数据文件
$dataCount = (Select-String -Path "data\regions\$regionId.js" -Pattern '"mp3_file":').Matches.Count
Write-Output "2. 数据记录数: $dataCount"

if ($audioCount -eq $dataCount) {
  Write-Output "   ✅ 音频文件与数据记录数量一致"
} else {
  Write-Output "   ❌ 数量不一致！请检查"
}

# 3. 检查 app.json 分包配置
$appJsonContent = Get-Content "app.json" -Raw
if ($appJsonContent -match "`"root`": `"$packageRoot`"") {
  Write-Output "3. ✅ app.json subPackages 已配置"
} else {
  Write-Output "3. ❌ app.json subPackages 未配置"
}

# 4. 检查 app.json 预加载配置
if ($appJsonContent -match "`"$packageName`"") {
  Write-Output "4. ✅ app.json preloadRule 已配置"
} else {
  Write-Output "4. ❌ app.json preloadRule 未配置"
}

# 5. 检查 audio-preload-guide.js
$guideContent = Get-Content "utils\audio-preload-guide.js" -Raw
if ($guideContent -match "'$regionId':\s*\{") {
  Write-Output "5. ✅ audio-preload-guide.js 已配置"
} else {
  Write-Output "5. ❌ audio-preload-guide.js 未配置"
}

# 6. 检查 audio-config.js（地区定义）
$configContent = Get-Content "utils\audio-config.js" -Raw
if ($configContent -match "id:\s*'$regionId'") {
  Write-Output "6. ✅ audio-config.js regions 已配置"
} else {
  Write-Output "6. ❌ audio-config.js regions 未配置"
}

# 7. 检查 audio-config.js（机场数据）
if ($configContent -match "regionId:\s*'$regionId'") {
  Write-Output "7. ✅ audio-config.js airports 已配置"
} else {
  Write-Output "7. ❌ audio-config.js airports 未配置"
}

# 8. 检查 audio-package-loader.js
$loaderContent = Get-Content "utils\audio-package-loader.js" -Raw
if ($loaderContent -match "'$regionId':\s*\{") {
  Write-Output "8. ✅ audio-package-loader.js 已配置"
} else {
  Write-Output "8. ❌ audio-package-loader.js 未配置"
}

# 9. 检查 audio-player/index.ts
$playerContent = Get-Content "pages\audio-player\index.ts" -Raw
if ($playerContent -match "'$regionId':\s*'/$packageRoot/'") {
  Write-Output "9. ✅ audio-player/index.ts 已配置"
} else {
  Write-Output "9. ❌ audio-player/index.ts 未配置"
}

# 10. 检查本地缓存系统集成
if ($playerContent -match "AudioCacheManager") {
  Write-Output "10. ✅ 播放器已集成本地缓存系统"
} else {
  Write-Output "10. ⚠️ 播放器未集成本地缓存系统（可选）"
}

Write-Output "========== 检查完成 =========="
```

---


## 配置文件详解

### app.json 完整结构

```json
{
  "pages": [...],           // 主包页面列表
  
  "subPackages": [          // 分包配置
    {
      "root": "packageXXX",       // 分包根目录
      "name": "xxxAudioPackage",  // 分包名称（用于 preloadRule）
      "pages": ["index"]          // 分包页面列表
    }
  ],
  
  "preloadRule": {          // 预加载规则
    "pages/xxx/index": {          // 触发预加载的页面路径
      "network": "all",           // 网络模式：all, wifi
      "packages": [               // 预加载的分包列表
        "packageXXX",
        "packageYYY"
      ]
    }
  },
  
  "window": {...},          // 窗口配置
  "tabBar": {...},          // TabBar 配置
  "permission": {...},      // 权限配置
  "requiredPrivateInfos": [...] // 隐私 API 声明
}
```

### audio-preload-guide.js 完整结构

```javascript
/**
 * 音频分包预加载引导配置管理器
 */

// TabBar 页面列表（禁止预加载音频）
var TABBAR_PAGES = [
  '/pages/search/index',
  '/pages/flight-calculator/index',
  '/pages/cockpit/index',
  '/pages/operations/index',
  '/pages/home/index'
];

function AudioPreloadGuide() {
  // 音频分包预加载页面映射配置
  this.preloadPageMapping = {
    '{regionId}': {
      packageName: 'package{CountryName}',
      regionName: '{国家}{城市}机场',
      flag: '{emoji}',
      preloadPage: '{页面路径}',
      preloadPageName: '{页面名称}',
      preloadPageIcon: '{emoji}',
      description: '{描述文案}'
    }
  };

  // 初始化本地存储系统
  this.initPreloadStorage();
}

// 核心方法
AudioPreloadGuide.prototype = {
  initPreloadStorage: function() {},      // 初始化存储
  getPreloadGuide: function(regionId) {}, // 获取引导信息
  generateGuideMessage: function(regionId) {}, // 生成引导消息
  checkPackagePreloaded: function(regionId) {}, // 检查预加载状态
  navigateToPreloadPage: function(regionId) {}, // 跳转预加载页面
  showPreloadGuideDialog: function(regionId) {}, // 显示引导弹窗
  markPackagePreloaded: function(regionId) {}, // 标记已预加载
  clearPreloadStatus: function(regionId) {} // 清除预加载状态
};

module.exports = AudioPreloadGuide;
```

---

## 数据验证清单

### ✅ 文件完整性检查

```bash
# 1. 检查音频文件数量
cd miniprogram/package{CountryName}
ls *.mp3 | wc -l

# 2. 检查数据文件记录数
grep -c '"mp3_file":' data/regions/{country}.js

# 3. 对比数量是否一致
# 音频文件数 = 数据记录数 ✅

# 4. 检查重复记录
grep '"mp3_file":' data/regions/{country}.js | sort | uniq -d

# 5. 检查缺失文件
# 对比数据文件中的 mp3_file 与实际文件列表
```

### ✅ 语法检查

```bash
# JavaScript 语法检查
cd miniprogram

# 检查分包入口文件
node -c package{CountryName}/index.js

# 检查数据文件
node -c data/regions/{country}.js

# 检查配置文件
node -c utils/audio-preload-guide.js

# 检查 app.json（使用 PowerShell）
try {
  $json = Get-Content app.json -Raw | ConvertFrom-Json
  Write-Output "✅ app.json JSON格式验证通过"
} catch {
  Write-Output "❌ app.json JSON格式错误"
}
```

### ✅ 预加载限制检查

```powershell
# 计算预加载页面总大小
$packages = @('packageRussia', 'packageAustralia', 'packageFrance', 
              'packageSingapore', 'packageUK')
$totalSize = 0
foreach ($pkg in $packages) {
  $size = (Get-ChildItem "$pkg\*.mp3" -File | 
           Measure-Object -Property Length -Sum).Sum / 1MB
  $totalSize += $size
  Write-Output "$pkg`: $([math]::Round($size, 2)) MB"
}
Write-Output "总预加载大小: $([math]::Round($totalSize, 2)) MB"

# 验证：总大小 < 2MB ✅
```

---

## 常见问题解决

### 问题 1：预加载超限（错误码 80058）

#### 症状
```
Error: 系统错误，错误码：80058
preloadRule [pages/xxx/index] source size XXX KB exceed max limit 2MB
```

#### 原因
- 单个页面预加载的分包总大小超过 2MB

#### 解决方案
```javascript
// 方案 1：压缩音频文件
ffmpeg -i input.mp3 -b:a 24k -ar 22050 -ac 1 output.mp3

// 方案 2：拆分到多个预加载页面
// 将大分包移到其他页面

// 方案 3：拆分分包
// 将一个大分包拆分为两个小分包
packageUK1 (0.8 MB) + packageUK2 (0.8 MB)
```

### 问题 2：数据文件与音频文件数量不一致

#### 检测方法
```bash
# 音频文件数量
audio_count=$(ls miniprogram/packageUK/*.mp3 | wc -l)

# 数据记录数量
data_count=$(grep -c '"mp3_file":' miniprogram/data/regions/uk.js)

# 对比
if [ $audio_count -eq $data_count ]; then
  echo "✅ 数量一致"
else
  echo "❌ 数量不一致: 音频$audio_count 数据$data_count"
fi
```

#### 解决方案
```bash
# 1. 找出重复记录
grep '"mp3_file":' data/regions/uk.js | sort | uniq -d

# 2. 找出缺失文件
# 列出数据文件中的所有 mp3_file
grep -oP '"mp3_file": "\K[^"]+' data/regions/uk.js > data_files.txt

# 列出实际音频文件
ls packageUK/*.mp3 | xargs -n 1 basename > audio_files.txt

# 对比
diff data_files.txt audio_files.txt
```

### 问题 3：音频播放时提示"分包未加载"

#### 原因
1. 用户未访问过预加载页面
2. 预加载配置错误
3. 分包名称不匹配

#### 解决方案
```javascript
// 1. 验证 regionId 与 packageName 匹配
// audio-preload-guide.js
'uk': {
  packageName: 'packageUK',  // 必须与 app.json 中的 root 一致
}

// app.json
{
  "root": "packageUK",  // 必须与 packageName 对应
  "name": "ukAudioPackage"
}

// 2. 验证预加载配置
// audio-preload-guide.js
'uk': {
  preloadPage: 'pages/communication-failure/index'  // 页面路径
}

// app.json
"pages/communication-failure/index": {
  "packages": ["packageUK"]  // 必须包含此分包
}
```

### 问题 4：JSON 格式错误

#### 常见错误
```json
// ❌ 错误 1：缺少逗号
{
  "label": "区调"
  "full_transcript": "..."  // 缺少逗号
}

// ❌ 错误 2：多余逗号
{
  "label": "区调",
  "full_transcript": "...",  // 最后一项不要逗号
}

// ❌ 错误 3：引号错误
{
  "label": '区调',  // 应该用双引号
  "mp3_file": "test.mp3"
}

// ✅ 正确格式
{
  "label": "区调",
  "full_transcript": "...",
  "translation_cn": "...",
  "mp3_file": "test.mp3"
}
```

#### 验证工具
```bash
# 使用 jq 验证 JSON 格式
cat data/regions/uk.js | grep -A 9999 "clips" | jq .

# 或使用 Node.js
node -e "require('./data/regions/uk.js')"
```

---

## 最佳实践建议

### 🎯 音频质量管理

```javascript
// 推荐的音频参数
const AUDIO_SETTINGS = {
  bitrate: '32-48 kbps',      // 比特率
  sampleRate: '22050 Hz',     // 采样率
  channels: 'mono',           // 单声道
  format: 'MP3',              // 格式
  targetSize: '< 50 KB/file'  // 单文件大小
};

// 压缩命令模板
const COMPRESS_COMMAND = `
  ffmpeg -i input.mp3 \\
    -b:a 32k \\           # 比特率
    -ar 22050 \\          # 采样率
    -ac 1 \\              # 单声道
    output.mp3
`;
```

### 🎯 数据组织规范

```javascript
// 1. 按飞行阶段排序
const PHASE_ORDER = [
  '放行',   // 1. Clearance Delivery
  '地面',   // 2. Ground Control
  '塔台',   // 3. Tower (Takeoff)
  '离场',   // 4. Departure
  '区调',   // 5. Area Control
  '进场',   // 6. Arrival
  '进近',   // 7. Approach
  '塔台',   // 8. Tower (Landing)
  '地面'    // 9. Ground Control (Taxi-in)
];

// 2. 同一航班的录音连续排列
// ✅ 推荐
China-Eastern-7551_xxx_1.mp3
China-Eastern-7551_xxx_2.mp3
China-Eastern-7551_xxx_3.mp3
Hainan-439_xxx_1.mp3
Hainan-439_xxx_2.mp3

// ❌ 不推荐
China-Eastern-7551_xxx_1.mp3
Hainan-439_xxx_1.mp3
China-Eastern-7551_xxx_2.mp3
```

### 🎯 版本管理

```javascript
// package{CountryName}/index.js 版本号规范

// 主版本号.次版本号.修订号
version: 'X.Y.Z'

// X: 主版本号（重大变更）
//    - 更换机场
//    - 完全重录
//    - 删除 >50% 文件

// Y: 次版本号（功能更新）
//    - 新增录音
//    - 更新部分录音
//    - 重新压缩

// Z: 修订号（bug修复）
//    - 修正翻译错误
//    - 修正文件名
//    - 元数据更新

// 示例
'1.0.0'  // 初始版本
'1.1.0'  // 新增10条录音
'1.1.1'  // 修正3条翻译
'2.0.0'  // 更换为另一个机场
```

### 🎯 文档化

```markdown
# 每个新增机场都应创建更新文档

## 文档结构
更新说明/
└── 新增{国家}{城市}音频分包_v{版本}.md
    ├── 音频分包信息
    ├── 航空公司覆盖
    ├── 音频内容分类
    ├── 特色陆空通话
    ├── 技术实现
    ├── 项目统计更新
    ├── 预加载配置更新
    ├── 验证清单
    └── 机场信息

## 模板
参考: 更新说明/新增英国伦敦音频分包_v2.8.1.md
```

---

## 快速参考表

### 文件路径对照

| 类型 | 路径 | 说明 |
|-----|------|------|
| 音频分包 | `miniprogram/package{CountryName}/` | 存放音频文件 |
| 分包入口 | `miniprogram/package{CountryName}/index.js` | 分包元数据 |
| 数据文件 | `miniprogram/data/regions/{country}.js` | 转录和翻译 |
| 全局配置 | `miniprogram/app.json` | 分包和预加载 |
| 引导配置 | `miniprogram/utils/audio-preload-guide.js` | 预加载映射 |
| 更新文档 | `更新说明/新增{国家}{城市}音频分包_v{版本}.md` | 版本记录 |

### 命令速查

```bash
# 统计音频数量
(Get-ChildItem "packageXXX\*.mp3" -File).Count

# 统计分包大小
$size = (Get-ChildItem "packageXXX\*.mp3" -File | 
         Measure-Object -Property Length -Sum).Sum / 1MB
[math]::Round($size, 2)

# 检查数据记录数
(Select-String -Path "data\regions\xxx.js" -Pattern '"mp3_file":').Matches.Count

# 检查重复记录
Select-String -Path "data\regions\xxx.js" -Pattern '"mp3_file": "(.*?)"' | 
  ForEach-Object { $_.Matches.Groups[1].Value } | 
  Group-Object | Where-Object { $_.Count -gt 1 }

# 语法检查
node -c packageXXX/index.js
node -c data/regions/xxx.js
```

---

## 附录

### A. 当前音频分包总览（v2.8.1）

| # | 国家/地区 | 分包名称 | 大小 | 文件数 | 预加载页面 |
|---|---------|---------|------|--------|-----------|
| 1 | 🇰🇷 韩国 | packageKorean | 1.74 MB | 121 | 录音播放 |
| 2 | 🇯🇵 日本 | packageJapan | 1.00 MB | 67 | 航线录音 |
| 3 | 🇺🇸 美国 | packageAmerica | 0.72 MB | 53 | 航线录音 |
| 4 | 🇦🇪 阿联酋 | packageUAE | 0.64 MB | 38 | 录音分类 |
| 5 | 🇬🇧 英国 | packageUK | 0.57 MB | 36 | 通信失效 |
| 6 | 🇮🇹 意大利 | packageItaly | 0.51 MB | 29 | 录音分类 |
| 7 | 🇵🇭 菲律宾 | packagePhilippines | 0.46 MB | 27 | 录音分类 |
| 8 | 🇹🇭 泰国 | packageThailand | 0.43 MB | 27 | 录音片段 |
| 9 | 🇱🇰 斯里兰卡 | packageSrilanka | 0.41 MB | 22 | 录音片段 |
| 10 | 🇹🇷 土耳其 | packageTurkey | 0.41 MB | 28 | 录音片段 |
| 11 | 🇷🇺 俄罗斯 | packageRussia | 0.38 MB | 23 | 通信失效 |
| 12 | 🇦🇺 澳大利亚 | packageAustralia | 0.35 MB | 20 | 通信失效 |
| 13 | 🇫🇷 法国 | packageFrance | 0.26 MB | 19 | 通信失效 |
| 14 | 🇸🇬 新加坡 | packageSingapore | 0.15 MB | 8 | 通信失效 |

**总计**: 14个音频分包，8.03 MB，374个音频文件

### B. 预加载页面配置总览

| 预加载页面 | 路径 | 图标 | 分包数 | 总大小 | 剩余空间 |
|-----------|------|------|--------|--------|---------|
| 录音播放 | pages/audio-player/index | 🎵 | 1 | 1.74 MB | 0.26 MB |
| 航线录音 | pages/airline-recordings/index | ✈️ | 2 | 1.72 MB | 0.28 MB |
| 录音分类 | pages/recording-categories/index | 📂 | 3 | 1.61 MB | 0.39 MB |
| 通信失效 | pages/communication-failure/index | 📡 | 5 | 1.71 MB | 0.29 MB |
| 录音片段 | pages/recording-clips/index | 🎬 | 3 | 1.25 MB | 0.75 MB |

---

**文档创建**: 2025-10-25  
**适用版本**: v2.8.1+  
**维护责任**: 开发团队  
**更新频率**: 随项目更新


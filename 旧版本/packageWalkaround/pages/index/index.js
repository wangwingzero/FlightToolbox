...
pageConfig.initImageCache = function() {
  ...
  try {
    ...
    this.imageCacheFs = null;
    this.imageCacheIndex = {};
...
pageConfig.getCachedImagePath = function(cacheKey) {
  ...
  if (!this.imageCacheFs || !this.imageCacheIndex) {
    return '';
  }
  ...
  try {
    this.imageCacheFs.accessSync(entry.path);
    return entry.path;
  } catch (error) {
    ...
    self.imageCachePromises[cacheKey] = new Promise(function(innerResolve, innerReject) {
      wx.getImageInfo({
        ...
        success: function(res) {
          ...
          try {
            self.imageCacheFs.accessSync(targetPath);
            self.imageCacheFs.unlinkSync(targetPath);
          } catch (removeError) {
            // 文件不存在时忽略
          }
...
pageConfig.restorePreloadedPackages = function(options) {
  ...
  if (typeof wx.loadSubpackage !== 'function') {
    console.warn('⚠️ 当前环境不支持 wx.loadSubpackage，跳过图片分包恢复');
    return;
  }

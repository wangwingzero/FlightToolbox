...
**更新时间**: 2024-12-24
**状态**: 原生模板广告已暂停，仅保留插屏广告和激励视频广告
**广告类型**: 插屏广告(Interstitial Ad) + 激励视频广告(Rewarded Video Ad)

---

## ⚠️ 2024-12-24 重大更新：原生模板广告暂停

### 变更说明

为优化用户体验，已暂停所有原生模板广告（横幅、竖版、全屏模板），仅保留：

- **插屏广告**：在5个TabBar页面展示，采用低频策略
- **激励视频广告**：用户主动观看，观看完成后获得1小时无广告奖励

### 技术实现

1. **全局开关**：在 `miniprogram/utils/app-config.js` 中添加 `nativeTemplateAdEnabled: false`
2. **条件渲染**：所有原生模板广告的WXML条件从 `wx:if="{{ !isAdFree }}"` 改为 `wx:if="{{ !isAdFree && nativeAdEnabled }}"`
3. **页面数据**：所有页面添加 `nativeAdEnabled` 数据项，在页面加载时读取配置

### 恢复方法

如需恢复原生模板广告，只需将 `app-config.js` 中的 `nativeTemplateAdEnabled` 设为 `true`。
...
| ---- | ---------------- | --------------------------- | ----------------- | ---- | ------ | ----------- |
| 1    | 通用插屏广告     | `adunit-1a29f1939a1c7864` | Interstitial Ad   | 优选 | 已开启 | 5个页面复用 |
| 2    | 资料查询激励视频 | `adunit-079d7e04aeba0625` | Rewarded Video Ad | 优选 | 已开启 | 1个页面     |

## 已暂停的广告位(3个原生模板)

| 序号 | 广告位名称 | 广告位ID                    | 广告类型           | 模式 | 状态   | 原使用次数 |
| ---- | ---------- | --------------------------- | ------------------ | ---- | ------ | ---------- |
| 1    | 全屏模板   | `adunit-d6c8a55bd3cb4fd1` | Native Template Ad | 优选 | 已暂停 | 11次       |
| 2    | 竖版       | `adunit-d7a3b71f5ce0afca` | Native Template Ad | 优选 | 已暂停 | 28次       |
| 3    | 横版       | `adunit-3a1bf3800fa937a2` | Native Template Ad | 优选 | 已暂停 | 64次       |

---

## 插屏广告策略参数（2024-12-24 优化）

| 参数         | 值              | 说明                       |
| ------------ | --------------- | -------------------------- |
| 最小时间间隔 | 5分钟           | 两次展示之间至少间隔5分钟  |
| 操作次数阈值 | 5次             | 至少切换5次TabBar才触发    |
| 会话限制     | 每30分钟最多5次 | 单个会话内的展示上限       |
| 每日上限     | 50次            | 每天最多展示50次           |
| 新用户保护   | 前20次操作      | 新用户前20次操作不展示广告 |
| 展示延迟     | 2秒             | 触发后延迟2秒展示          |

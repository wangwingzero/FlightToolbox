# 绕机检查分包预加载完整指南

> **重要提示**：本文档详细记录了绕机检查模块的分包预加载机制和新机型添加流程。
> **目的**：让任何人看完后能够快速添加新机型（如B737），无需重复摸索。

---

## 📖 目录

1. [微信小程序分包预加载机制](#1-微信小程序分包预加载机制)
2. [当前A330绕机检查架构](#2-当前a330绕机检查架构)
3. [分包预加载引导系统（8步配置法）](#3-分包预加载引导系统8步配置法)
4. [添加新机型完整流程](#4-添加新机型完整流程)
5. [手动清理预加载状态](#5-手动清理预加载状态)
6. [故障排查指南](#6-故障排查指南)
7. [验证测试清单](#7-验证测试清单)

---

## 1. 微信小程序分包预加载机制

### 1.1 为什么需要分包？

**微信小程序硬性限制**：
- 单个分包最大2MB（2048KB）
- 主包 + 所有分包总大小最大20MB

**绕机检查图片资源**：
- A330机型：24个区域，132张PNG图片，总大小约4.3MB
- 必须分成多个分包（目前是6个分包，每个约600-800KB）

### 1.2 微信小程序的预加载机制（真机实现原理）

#### 方式1：app.json的preloadRule（自动预加载）

**配置示例**：
```json
{
  "preloadRule": {
    "packageO/sunrise-sunset/index": {
      "packages": ["walkaroundImages2Package"],
      "network": "all"
    }
  }
}
```

**真机工作原理**：
1. 用户访问"日出日落"页面（`packageO/sunrise-sunset/index`）
2. 微信在后台自动下载`walkaroundImages2Package`分包
3. 下载完成后，分包内容存储在小程序缓存中
4. 下次访问绕机检查区域5-8时，图片直接从缓存加载

**关键约束**：
- ⚠️ **单页面预加载总大小必须<2MB**（所有分包加起来）
- ⚠️ **禁止在TabBar主页面预加载**（会影响启动速度）
- ⚠️ **network: "all"** 表示WiFi和移动网络都预加载

#### 方式2：手动加载（wx.loadSubpackage）

**代码示例**：
```javascript
wx.loadSubpackage({
  name: 'walkaroundImages2Package',
  success: function(res) {
    console.log('分包加载成功');
  },
  fail: function(res) {
    console.error('分包加载失败', res);
  }
});
```

**缺点**：需要用户主动触发，体验不如自动预加载。

### 1.3 我们采用的方案：引导用户预加载

**核心思路**：
1. 区域1-4的图片在绕机检查页面自动预加载（本页面就是预加载页面）
2. 区域5-24的图片分散到5个不同的功能页面预加载
3. 用户首次访问区域5+时，弹出引导对话框，引导访问对应页面
4. 用户访问预加载页面后，微信自动下载图片分包
5. 下次访问该区域时，图片直接从缓存加载，不再弹窗

**优点**：
- ✅ 绕过2MB单页面预加载限制
- ✅ 不影响TabBar页面启动速度
- ✅ 用户体验好（只需访问一次）
- ✅ 完全离线可用（访问过后）

---

## 2. 当前A330绕机检查架构

### 2.1 分包结构

```
packageWalkaround (主分包)
   ├── pages/index/              # 绕机检查主页面
   ├── data/a330/                # A330数据（areas.js, checkitems.js）
   └── utils/                    # 工具函数

packageWalkaroundImages1 (600-800KB)
   └── images1/                  # 区域1-4图片（预加载页面：绕机检查本页）

packageWalkaroundImages2 (600-800KB)
   └── images2/                  # 区域5-8图片（预加载页面：日出日落）

packageWalkaroundImages3 (600-800KB)
   └── images3/                  # 区域9-12图片（预加载页面：个人检查单）

packageWalkaroundImages4 (600-800KB)
   └── images4/                  # 区域13-16图片（预加载页面：飞行时间分摊）

packageWalkaroundImages5 (600-800KB)
   └── images5/                  # 区域17-20图片（预加载页面：体检标准）

packageWalkaroundImages6 (600-800KB)
   └── images6/                  # 区域21-24图片（预加载页面：通信规范）
```

### 2.2 预加载引导系统架构

```
用户点击区域5
    ↓
检查预加载状态（walkaround-preload-guide.js）
    ↓
未预加载 → 显示引导对话框
    ↓
引导访问"日出日落"页面
    ↓
用户访问"日出日落"
    ↓
微信自动预加载walkaroundImages2Package
    ↓
标记区域5-8为已预加载
    ↓
下次访问区域5-8，直接显示图片，不再弹窗
```

---

## 3. 分包预加载引导系统（8步配置法）

**⚠️ 历史教训**：最初只做了5步，导致UK和Chinese Taipei音频无法播放。必须完成全部8步！

### 步骤1：创建分包目录和图片文件

```bash
# 示例：为区域5-8创建分包
mkdir -p miniprogram/packageWalkaroundImages2/images2
mkdir -p miniprogram/packageWalkaroundImages2/pages/placeholder

# 复制图片到images2目录
# （假设你已经准备好了区域5-8的所有组件图片）
cp 源目录/*.png miniprogram/packageWalkaroundImages2/images2/

# 创建占位页面（微信要求分包必须有页面）
cat > miniprogram/packageWalkaroundImages2/pages/placeholder/index.json <<EOF
{
  "usingComponents": {}
}
EOF

cat > miniprogram/packageWalkaroundImages2/pages/placeholder/index.js <<EOF
Page({
  data: {}
});
EOF

cat > miniprogram/packageWalkaroundImages2/pages/placeholder/index.wxml <<EOF
<view>Placeholder page for packageWalkaroundImages2</view>
EOF

cat > miniprogram/packageWalkaroundImages2/pages/placeholder/index.wxss <<EOF
/* Placeholder styles */
EOF
```

**验证**：
```bash
# 检查目录结构
ls -la miniprogram/packageWalkaroundImages2/images2/
ls -la miniprogram/packageWalkaroundImages2/pages/placeholder/

# 检查分包大小（必须<2MB）
du -sh miniprogram/packageWalkaroundImages2/
```

### 步骤2：更新app.json（添加分包配置）

**文件位置**：`miniprogram/app.json`

```json
{
  "subPackages": [
    // ... 其他分包

    {
      "root": "packageWalkaroundImages2",
      "name": "walkaroundImages2Package",
      "pages": ["pages/placeholder/index"]
    }
  ],

  "preloadRule": {
    // ... 其他预加载规则

    "packageO/sunrise-sunset/index": {
      "packages": ["walkaroundImages2Package"],
      "network": "all"
    }
  }
}
```

**说明**：
- `root`: 分包目录名
- `name`: 分包名称（驼峰命名）
- `packages`: 要预加载的分包名称数组
- `network: "all"`: WiFi和移动网络都预加载

**⚠️ 关键约束**：
- 单页面预加载的所有分包总大小必须<2MB
- 不能在TabBar页面配置预加载

### 步骤3：更新utils/walkaround-preload-guide.js

**文件位置**：`miniprogram/utils/walkaround-preload-guide.js`

**添加区域映射配置**：
```javascript
this.areaPackageMapping = {
  '1-4': {
    packageName: 'packageWalkaroundImages1',
    areaRange: [1, 2, 3, 4],
    areaNames: '驾驶舱前部、前起落架、前货舱、前登机门',
    preloadPage: 'packageWalkaround/pages/index/index',
    preloadPageName: '绕机检查',
    preloadPageIcon: '✈️'
  },
  '5-8': {
    packageName: 'packageWalkaroundImages2',
    areaRange: [5, 6, 7, 8],
    areaNames: '左前门、左发动机、左主起落架、左翼',
    preloadPage: 'packageO/sunrise-sunset/index',  // ← 预加载页面路径
    preloadPageName: '日出日落',                    // ← 页面名称
    preloadPageIcon: '🌅'                           // ← 页面图标
  }
  // ... 其他区域
};
```

**关键字段说明**：
- `packageName`: 图片分包的目录名
- `areaRange`: 区域ID范围数组
- `areaNames`: 区域中文名称（用于对话框显示）
- `preloadPage`: 预加载页面路径（必须与app.json的preloadRule键一致）
- `preloadPageName`: 预加载页面名称（显示在引导对话框中）
- `preloadPageIcon`: 页面图标（emoji）

### 步骤4：更新预加载页面（标记分包为已加载）

**文件位置**：`miniprogram/packageO/sunrise-sunset/index.ts`

```typescript
// 在页面顶部引入预加载引导
var WalkaroundPreloadGuide = require('../../utils/walkaround-preload-guide.js');

Page({
  customOnLoad: function() {
    // ... 其他初始化代码

    // 标记绕机检查区域5-8的图片分包为已预加载
    try {
      var preloadGuide = new WalkaroundPreloadGuide();
      preloadGuide.markPackagePreloaded('5-8');  // ← 区域范围键
      console.log('✅ 已标记绕机检查区域5-8的图片分包为已预加载');
    } catch (error) {
      console.error('❌ 标记绕机检查图片分包失败:', error);
    }

    // ... 其他初始化代码
  }
});
```

**⚠️ 关键点**：
- `'5-8'` 必须与 `walkaround-preload-guide.js` 中的键完全一致
- 必须在 `customOnLoad` 或 `onLoad` 中调用
- 每个预加载页面只标记一个区域范围

### 步骤5：更新绕机检查主页面（集成引导系统）

**文件位置**：`miniprogram/packageWalkaround/pages/index/index.js`

```javascript
// 在页面顶部引入预加载引导
var WalkaroundPreloadGuide = require('../../../utils/walkaround-preload-guide.js');

Page({
  customOnLoad: function() {
    // 初始化预加载引导系统
    this.preloadGuide = new WalkaroundPreloadGuide();

    // 标记区域1-4为已预加载（本页面自动预加载）
    this.preloadGuide.markPackagePreloaded('1-4');
  },

  // 用户点击区域时检查预加载状态
  selectAreaAndShowPopup: function(areaId) {
    var self = this;
    var area = this.data.areaList.find(function(item) {
      return item.id === areaId;
    });

    if (!area) {
      return;
    }

    // 检查该区域对应的图片分包是否已预加载
    this.preloadGuide.checkPackagePreloaded(areaId).then(function(isPreloaded) {
      if (!isPreloaded) {
        // 未预加载，显示引导对话框
        console.log('🚨 区域 ' + areaId + ' 的图片分包未预加载，显示引导对话框');
        self.preloadGuide.showPreloadGuideDialog(areaId).then(function(navigated) {
          if (!navigated) {
            // 用户选择稍后再说，仍然显示详情（但图片可能加载失败）
            console.log('⚠️ 用户选择稍后再说，仍然显示详情页面');
            self.showAreaDetails(area, areaId);
          }
        });
      } else {
        // 已预加载，直接显示详情
        console.log('✅ 区域 ' + areaId + ' 的图片分包已预加载，直接显示详情');
        self.showAreaDetails(area, areaId);
      }
    });
  }
});
```

### 步骤6：确保图片路径配置正确

**文件位置**：`miniprogram/packageWalkaround/pages/index/index.js`

```javascript
// 根据区域ID获取图片路径
function getImagePathByArea(areaId) {
  if (areaId >= 1 && areaId <= 4) {
    return '/packageWalkaroundImages1/images1/';
  } else if (areaId >= 5 && areaId <= 8) {
    return '/packageWalkaroundImages2/images2/';
  } else if (areaId >= 9 && areaId <= 12) {
    return '/packageWalkaroundImages3/images3/';
  } else if (areaId >= 13 && areaId <= 16) {
    return '/packageWalkaroundImages4/images4/';
  } else if (areaId >= 17 && areaId <= 20) {
    return '/packageWalkaroundImages5/images5/';
  } else if (areaId >= 21 && areaId <= 24) {
    return '/packageWalkaroundImages6/images6/';
  }
  return '/packageWalkaroundImages1/images1/';  // 默认
}
```

**⚠️ 重要**：
- 路径必须以 `/` 开头（绝对路径）
- 路径必须以 `/` 结尾
- 区域范围必须连续，不能有遗漏

### 步骤7：验证微信开发者工具编译

```bash
# 在微信开发者工具中：
# 1. 点击"编译"按钮
# 2. 检查控制台无错误
# 3. 检查"详情 -> 本地代码 -> 代码包大小详情"
#    - 确认新分包已列出
#    - 确认分包大小 < 2MB
```

### 步骤8：真机测试验证

**测试步骤**：
1. 清理预加载状态（我的首页 → 应用信息 → 清理预加载状态）
2. 进入绕机检查页面
3. 点击区域5
4. 应该弹出引导对话框："要查看区域5，请先访问🌅日出日落页面"
5. 点击"立即前往"
6. 进入日出日落页面（微信自动预加载图片分包）
7. 返回绕机检查页面
8. 再次点击区域5
9. 应该直接显示图片，不再弹窗

**⚠️ 8步全部完成才算配置成功！**

---

## 4. 添加新机型完整流程

### 场景：添加B737绕机检查

假设B737有20个检查区域，100张图片，总大小约3.5MB，需要分成5个分包。

### 4.1 数据准备阶段

#### 步骤1：准备检查区域数据

**创建文件**：`miniprogram/packageWalkaround/data/b737/areas.js`

```javascript
var areas = [
  {
    id: 1,
    name_zh: '机头',
    name_en: 'Nose',
    sequence: 1,
    hotspot: {
      x: 150,  // Canvas热点x坐标
      y: 100,  // Canvas热点y坐标
      r: 30    // 热点半径
    }
  },
  // ... 其他19个区域
];

module.exports = {
  areas: areas
};
```

#### 步骤2：准备检查项数据

**创建文件**：`miniprogram/packageWalkaround/data/b737/checkitems.js`

```javascript
var checkItems = [
  {
    id: 'b737_area1_item1',
    areaId: 1,
    sequence: 1,
    componentId: 'radome',
    requirement_zh: '检查状态',
    requirement_en: 'Check condition',
    notes_zh: '无损坏',
    notes_en: 'No damage'
  },
  // ... 其他检查项
];

module.exports = {
  checkItems: checkItems
};
```

#### 步骤3：准备部件定义数据

**创建文件**：`miniprogram/packageWalkaround/data/b737/components.js`

```javascript
var components = [
  {
    id: 'radome',
    name_zh: '雷达罩',
    name_en: 'Radome',
    category: 'fuselage'
  },
  // ... 其他部件
];

module.exports = {
  components: components
};
```

#### 步骤4：准备飞机Canvas图片

**文件路径**：`miniprogram/packageWalkaround/images/b737_aircraft.png`

- 尺寸建议：800x600px
- 格式：PNG透明背景
- 包含所有检查区域的示意图

### 4.2 图片分包创建阶段

#### 步骤1：规划分包数量

```
总图片大小：3.5MB
单分包限制：1.8MB（留10%余量）
分包数量：ceil(3.5 / 1.8) = 2个分包

但为了均衡，我们按区域划分：
- 20个区域 ÷ 5个分包 = 每分包4个区域

分包1：区域1-4   (700KB)
分包2：区域5-8   (700KB)
分包3：区域9-12  (700KB)
分包4：区域13-16 (700KB)
分包5：区域17-20 (700KB)
```

#### 步骤2：创建5个图片分包

**使用PowerShell脚本自动创建**：

```powershell
# create-b737-packages.ps1
$packages = @(
    @{num=7; areas="1-4"},
    @{num=8; areas="5-8"},
    @{num=9; areas="9-12"},
    @{num=10; areas="13-16"},
    @{num=11; areas="17-20"}
)

foreach ($pkg in $packages) {
    $num = $pkg.num
    $dir = "miniprogram/packageWalkaroundImages$num"

    # 创建目录
    New-Item -ItemType Directory -Force -Path "$dir/images$num"
    New-Item -ItemType Directory -Force -Path "$dir/pages/placeholder"

    # 创建占位页面
    @"
{
  "usingComponents": {}
}
"@ | Out-File -Encoding UTF8 "$dir/pages/placeholder/index.json"

    @"
Page({
  data: {}
});
"@ | Out-File -Encoding UTF8 "$dir/pages/placeholder/index.js"

    @"
<view>Placeholder page for packageWalkaroundImages$num</view>
"@ | Out-File -Encoding UTF8 "$dir/pages/placeholder/index.wxml"

    @"
/* Placeholder styles */
"@ | Out-File -Encoding UTF8 "$dir/pages/placeholder/index.wxss"

    Write-Host "✅ 创建完成: packageWalkaroundImages$num (区域 $($pkg.areas))"
}

Write-Host "`n🎉 所有B737图片分包创建完成！"
```

运行脚本：
```powershell
cd D:\FlightToolbox
.\create-b737-packages.ps1
```

#### 步骤3：复制图片到各分包

**手动复制**：
```powershell
# 复制区域1-4的图片到分包7
Copy-Item "源目录/area1-4/*.png" "miniprogram/packageWalkaroundImages7/images7/"

# 复制区域5-8的图片到分包8
Copy-Item "源目录/area5-8/*.png" "miniprogram/packageWalkaroundImages8/images8/"

# ... 其他分包
```

**验证大小**：
```powershell
Get-ChildItem "miniprogram/packageWalkaroundImages*" -Directory | ForEach-Object {
    $size = (Get-ChildItem $_.FullName -Recurse | Measure-Object -Property Length -Sum).Sum / 1KB
    Write-Host "$($_.Name): $([math]::Round($size, 2)) KB"
}
```

### 4.3 分包配置阶段

#### 步骤1：更新app.json

**在`subPackages`数组中添加**：

```json
{
  "subPackages": [
    // ... 现有分包

    {
      "root": "packageWalkaroundImages7",
      "name": "walkaroundImages7Package",
      "pages": ["pages/placeholder/index"]
    },
    {
      "root": "packageWalkaroundImages8",
      "name": "walkaroundImages8Package",
      "pages": ["pages/placeholder/index"]
    },
    {
      "root": "packageWalkaroundImages9",
      "name": "walkaroundImages9Package",
      "pages": ["pages/placeholder/index"]
    },
    {
      "root": "packageWalkaroundImages10",
      "name": "walkaroundImages10Package",
      "pages": ["pages/placeholder/index"]
    },
    {
      "root": "packageWalkaroundImages11",
      "name": "walkaroundImages11Package",
      "pages": ["pages/placeholder/index"]
    }
  ]
}
```

#### 步骤2：更新preloadRule

**选择5个预加载页面**（每个页面预加载一个分包）：

```json
{
  "preloadRule": {
    // A330现有规则
    "packageO/sunrise-sunset/index": {
      "packages": ["walkaroundImages2Package"],
      "network": "all"
    },

    // B737新增规则
    "packageO/fuel-calculator/index": {
      "packages": ["walkaroundImages7Package"],
      "network": "all"
    },
    "packageO/weight-balance/index": {
      "packages": ["walkaroundImages8Package"],
      "network": "all"
    },
    "packageO/takeoff-performance/index": {
      "packages": ["walkaroundImages9Package"],
      "network": "all"
    },
    "packageO/landing-performance/index": {
      "packages": ["walkaroundImages10Package"],
      "network": "all"
    },
    "packageO/alternate-selection/index": {
      "packages": ["walkaroundImages11Package"],
      "network": "all"
    }
  }
}
```

**⚠️ 选择预加载页面的原则**：
1. 不能是TabBar页面
2. 页面访问频率较高（用户容易访问到）
3. 单页面预加载总大小<2MB
4. 尽量选择功能相关的页面（如燃油计算器对应B737燃油相关区域）

### 4.4 预加载引导系统配置

#### 步骤1：更新walkaround-preload-guide.js

**添加B737区域映射**：

```javascript
this.areaPackageMapping = {
  // A330现有映射
  '1-4': {
    packageName: 'packageWalkaroundImages1',
    areaRange: [1, 2, 3, 4],
    areaNames: '...',
    preloadPage: 'packageWalkaround/pages/index/index',
    preloadPageName: '绕机检查',
    preloadPageIcon: '✈️',
    modelId: 'a330'  // ← 添加机型标识
  },

  // B737新增映射
  'b737-1-4': {
    packageName: 'packageWalkaroundImages7',
    areaRange: [1, 2, 3, 4],
    areaNames: '机头、前起落架、前货舱、前门',
    preloadPage: 'packageO/fuel-calculator/index',
    preloadPageName: '燃油计算器',
    preloadPageIcon: '⛽',
    modelId: 'b737'
  },
  'b737-5-8': {
    packageName: 'packageWalkaroundImages8',
    areaRange: [5, 6, 7, 8],
    areaNames: '左前门、左发动机、左主起落架、左翼',
    preloadPage: 'packageO/weight-balance/index',
    preloadPageName: '重量平衡',
    preloadPageIcon: '⚖️',
    modelId: 'b737'
  }
  // ... 其他B737区域
};
```

#### 步骤2：更新预加载页面

**为每个预加载页面添加标记代码**：

```javascript
// packageO/fuel-calculator/index.js
var WalkaroundPreloadGuide = require('../../utils/walkaround-preload-guide.js');

Page({
  customOnLoad: function() {
    // 标记B737区域1-4的图片分包为已预加载
    try {
      var preloadGuide = new WalkaroundPreloadGuide();
      preloadGuide.markPackagePreloaded('b737-1-4');  // ← B737区域键
      console.log('✅ 已标记B737区域1-4的图片分包为已预加载');
    } catch (error) {
      console.error('❌ 标记失败:', error);
    }
  }
});
```

**重复以上代码到其他4个预加载页面**，分别标记：
- `'b737-5-8'`
- `'b737-9-12'`
- `'b737-13-16'`
- `'b737-17-20'`

### 4.5 绕机检查页面更新

#### 步骤1：支持机型切换

**更新`packageWalkaround/pages/index/index.js`**：

```javascript
Page({
  data: {
    modelId: 'a330',  // 当前机型（可切换为'b737'）
    // ...
  },

  customOnLoad: function(options) {
    // 从URL参数获取机型（如有）
    var modelId = options.model || 'a330';
    this.setData({ modelId: modelId });

    // 加载对应机型的数据
    this.loadModelData(modelId);

    // 初始化预加载引导系统
    this.preloadGuide = new WalkaroundPreloadGuide();

    // 标记当前机型区域1-4为已预加载
    var rangeKey = modelId === 'b737' ? 'b737-1-4' : '1-4';
    this.preloadGuide.markPackagePreloaded(rangeKey);
  },

  loadModelData: function(modelId) {
    var dataPath = '../../data/' + modelId + '/';
    var Areas = require(dataPath + 'areas.js');
    var CheckItems = require(dataPath + 'checkitems.js');
    var Components = require(dataPath + 'components.js');

    this.setData({
      areas: Areas.areas,
      checkItems: CheckItems.checkItems,
      components: Components.components
    });
  },

  // 获取图片路径（根据机型）
  getImagePathByArea: function(areaId) {
    var modelId = this.data.modelId;

    if (modelId === 'b737') {
      // B737图片路径
      if (areaId >= 1 && areaId <= 4) {
        return '/packageWalkaroundImages7/images7/';
      } else if (areaId >= 5 && areaId <= 8) {
        return '/packageWalkaroundImages8/images8/';
      } else if (areaId >= 9 && areaId <= 12) {
        return '/packageWalkaroundImages9/images9/';
      } else if (areaId >= 13 && areaId <= 16) {
        return '/packageWalkaroundImages10/images10/';
      } else if (areaId >= 17 && areaId <= 20) {
        return '/packageWalkaroundImages11/images11/';
      }
    } else {
      // A330图片路径（现有逻辑）
      if (areaId >= 1 && areaId <= 4) {
        return '/packageWalkaroundImages1/images1/';
      }
      // ... 其他A330路径
    }

    return '/packageWalkaroundImages1/images1/';  // 默认
  }
});
```

### 4.6 添加机型选择入口

**在绕机检查主页面添加机型切换按钮**：

```xml
<!-- packageWalkaround/pages/index/index.wxml -->
<view class="model-selector">
  <button bind:tap="switchToA330">A330</button>
  <button bind:tap="switchToB737">B737</button>
</view>
```

```javascript
// packageWalkaround/pages/index/index.js
Page({
  switchToA330: function() {
    wx.reLaunch({
      url: '/packageWalkaround/pages/index/index?model=a330'
    });
  },

  switchToB737: function() {
    wx.reLaunch({
      url: '/packageWalkaround/pages/index/index?model=b737'
    });
  }
});
```

---

## 5. 手动清理预加载状态

### 5.1 为什么需要手动清理？

**开发调试场景**：
- 测试预加载引导对话框是否正常显示
- 验证新增分包的预加载流程
- 修复预加载引导系统bug后重新测试

**不自动清理的原因**：
- 真机上不应该自动清理（影响用户体验）
- 开发工具和真机的存储隔离，需要手动控制

### 5.2 清理功能位置

**我的首页 → 应用信息 → 清理预加载状态**

### 5.3 清理流程

1. 打开小程序，进入"我的首页"（TabBar第5个）
2. 滚动到"应用信息"区域
3. 点击"清理预加载状态"（橙红色背景）
4. 确认对话框：
   ```
   ⚠️ 开发调试功能

   此功能将清除绕机检查图片的所有预加载状态，仅用于开发调试。

   清理后，再次访问绕机检查的区域5-24时将重新显示预加载引导对话框。

   通常只在测试引导系统时使用，确定要清理吗？

   [取消]  [确定清理]
   ```
5. 点击"确定清理"
6. 显示"清理成功"提示
7. 返回绕机检查页面，点击区域5+，应该重新显示引导对话框

### 5.4 清理实现原理

```javascript
// pages/home/index.js
clearPreloadStatus: function() {
  wx.showModal({
    title: '⚠️ 开发调试功能',
    content: '此功能将清除绕机检查图片的所有预加载状态...',
    confirmText: '确定清理',
    confirmColor: '#ff6b6b',
    success: function(res) {
      if (res.confirm) {
        // 清除存储
        wx.setStorageSync('flight_toolbox_walkaround_preload_status', {});
        wx.showToast({
          title: '清理成功',
          icon: 'success'
        });
      }
    }
  });
}
```

**清除的数据**：
```javascript
// 预加载状态存储格式
{
  '1-4': 1762224701882,      // 时间戳（标记加载时间）
  '5-8': 1762223528370,
  '9-12': 1762223497949,
  'b737-1-4': 1762224800000,
  // ...
}

// 清理后变为空对象
{}
```

---

## 6. 故障排查指南

### 6.1 引导对话框不显示

**症状**：点击区域5+时，直接显示详情页面，但图片404，没有引导对话框

**排查步骤**：

1. **检查预加载状态是否被错误标记**
   ```javascript
   // 在微信开发者工具控制台输入
   wx.getStorageSync('flight_toolbox_walkaround_preload_status')
   ```
   如果返回 `{ '5-8': 1762223528370 }`，说明区域5-8已被标记为预加载。

   **解决**：清理预加载状态（我的首页 → 清理预加载状态）

2. **检查walkaround-preload-guide.js配置**
   ```javascript
   // 确认区域5-8的配置存在
   this.areaPackageMapping = {
     '5-8': { ... }  // ← 必须存在
   };
   ```

3. **检查绕机检查页面是否调用检查方法**
   ```javascript
   selectAreaAndShowPopup: function(areaId) {
     // 必须调用checkPackagePreloaded
     this.preloadGuide.checkPackagePreloaded(areaId).then(...);
   }
   ```

### 6.2 引导对话框显示，但按钮文字过长报错

**症状**：控制台报错
```
wx.showModal:fail confirmText length should not larger than 4 Chinese characters
```

**原因**：微信`wx.showModal`的`confirmText`最多4个汉字

**解决**：
```javascript
// ❌ 错误
confirmText: '前往 🌅 日出日落'  // 7个字符

// ✅ 正确
confirmText: '立即前往'  // 4个字符
```

### 6.3 点击"立即前往"后，图片仍然404

**症状**：访问了预加载页面，返回后图片仍然加载失败

**排查步骤**：

1. **检查app.json的preloadRule配置**
   ```json
   {
     "preloadRule": {
       "packageO/sunrise-sunset/index": {  // ← 路径必须正确
         "packages": ["walkaroundImages2Package"],  // ← 分包名称必须正确
         "network": "all"
       }
     }
   }
   ```

2. **检查分包名称是否一致**
   ```json
   // app.json的subPackages
   {
     "root": "packageWalkaroundImages2",
     "name": "walkaroundImages2Package",  // ← 这里
     "pages": ["pages/placeholder/index"]
   }

   // app.json的preloadRule
   {
     "packageO/sunrise-sunset/index": {
       "packages": ["walkaroundImages2Package"],  // ← 必须一致
       "network": "all"
     }
   }
   ```

3. **检查图片路径是否正确**
   ```javascript
   // 正确的路径格式
   '/packageWalkaroundImages2/images2/component.png'

   // ❌ 错误示例
   'packageWalkaroundImages2/images2/component.png'  // 缺少开头的/
   '/packageWalkaroundImages2/images2'  // 缺少结尾的/
   ```

4. **检查图片文件是否存在**
   ```bash
   # Windows PowerShell
   Test-Path "miniprogram/packageWalkaroundImages2/images2/component.png"
   ```

5. **检查分包大小是否超限**
   ```powershell
   # Windows PowerShell
   $size = (Get-ChildItem "miniprogram/packageWalkaroundImages2" -Recurse |
            Measure-Object -Property Length -Sum).Sum / 1KB
   Write-Host "$size KB"

   # 必须 < 2048KB
   ```

### 6.4 真机测试时图片正常，开发工具图片404

**原因**：开发工具的分包加载机制与真机不同

**解决**：以真机测试为准，开发工具的404可忽略

### 6.5 添加新分包后编译失败

**症状**：
```
Error: subPackage "packageWalkaroundImagesX" does not have any pages
```

**原因**：缺少占位页面

**解决**：
```bash
# 创建占位页面目录
mkdir -p miniprogram/packageWalkaroundImagesX/pages/placeholder

# 创建4个文件
# index.json, index.js, index.wxml, index.wxss
```

---

## 7. 验证测试清单

### 7.1 开发工具编译测试

- [ ] 微信开发者工具编译无错误
- [ ] 控制台无warning（可忽略Vant组件的getSystemInfoSync警告）
- [ ] "详情 → 本地代码 → 代码包大小详情"显示所有分包
- [ ] 每个分包大小 < 2MB
- [ ] preloadRule配置显示正确

### 7.2 功能测试（开发工具）

- [ ] 绕机检查主页面正常加载
- [ ] Canvas飞机图正常显示
- [ ] 点击热点区域正确识别
- [ ] 点击区域1-4直接显示图片（无引导对话框）
- [ ] 点击区域5-8显示引导对话框
- [ ] 对话框文字正确，按钮文字"立即前往"
- [ ] 点击"立即前往"正确跳转到预加载页面
- [ ] 预加载页面控制台输出"✅ 已标记..."
- [ ] 返回绕机检查页面，再次点击区域5-8
- [ ] 第二次点击不再显示对话框，直接显示详情

### 7.3 清理功能测试

- [ ] 进入"我的首页"
- [ ] 滚动到"应用信息"区域
- [ ] 可以看到"清理预加载状态"按钮（橙红色背景）
- [ ] 点击按钮显示确认对话框
- [ ] 对话框内容完整，警告清晰
- [ ] 点击"取消"，对话框关闭，无操作
- [ ] 点击"确定清理"，显示"清理成功"
- [ ] 清理后，重新测试区域5-8，应该再次显示引导对话框

### 7.4 真机测试（Android）

- [ ] 扫码打开小程序
- [ ] 进入绕机检查页面
- [ ] 点击区域1-4，图片正常显示
- [ ] 点击区域5-8，显示引导对话框
- [ ] 点击"立即前往"，跳转到预加载页面
- [ ] 等待3-5秒（微信后台下载分包）
- [ ] 返回绕机检查页面，再次点击区域5-8
- [ ] 图片正常显示，无引导对话框
- [ ] 开启飞行模式
- [ ] 再次访问区域5-8，图片从缓存加载正常显示

### 7.5 真机测试（iOS）

- [ ] 重复Android测试步骤
- [ ] 特别注意iOS的缓存机制
- [ ] 验证飞行模式下图片正常显示

### 7.6 回归测试

**添加新机型后必须验证**：

- [ ] A330绕机检查功能正常（未受影响）
- [ ] A330预加载引导正常（未受影响）
- [ ] B737绕机检查功能正常
- [ ] B737预加载引导正常
- [ ] 机型切换功能正常
- [ ] 清理预加载状态对两个机型都生效

---

## 8. 核心文件清单

### 8.1 预加载引导系统核心文件

| 文件路径 | 作用 | 关键内容 |
|---------|------|---------|
| `miniprogram/utils/walkaround-preload-guide.js` | 预加载引导管理器 | 区域映射、对话框显示、状态管理 |
| `miniprogram/app.json` | 小程序配置 | subPackages、preloadRule |
| `miniprogram/packageWalkaround/pages/index/index.js` | 绕机检查主页面 | 引导系统集成、区域点击检查 |

### 8.2 预加载页面文件（需要添加标记代码）

| 文件路径 | 负责区域 | 标记键 |
|---------|---------|--------|
| `packageO/sunrise-sunset/index.ts` | 区域5-8 | `'5-8'` |
| `packageO/personal-checklist/index.ts` | 区域9-12 | `'9-12'` |
| `packageO/flight-time-share/index.ts` | 区域13-16 | `'13-16'` |
| `packageMedical/index.js` | 区域17-20 | `'17-20'` |
| `pages/communication-rules/index.ts` | 区域21-24 | `'21-24'` |

### 8.3 图片分包目录结构

```
packageWalkaroundImages1/
├── images1/
│   ├── component1.png
│   ├── component2.png
│   └── ...
└── pages/
    └── placeholder/
        ├── index.js
        ├── index.json
        ├── index.wxml
        └── index.wxss

packageWalkaroundImages2/
├── images2/
│   └── ...
└── pages/
    └── placeholder/
        └── ...

... (共6个分包)
```

---

## 9. 快速参考命令

### 9.1 检查分包大小

```powershell
# PowerShell
Get-ChildItem "D:\FlightToolbox\miniprogram\packageWalkaroundImages*" -Directory | ForEach-Object {
    $size = (Get-ChildItem $_.FullName -Recurse | Measure-Object -Property Length -Sum).Sum / 1KB
    $color = if ($size -gt 2048) { "Red" } elseif ($size -gt 1840) { "Yellow" } else { "Green" }
    Write-Host "$($_.Name): $([math]::Round($size, 2)) KB" -ForegroundColor $color
}
```

### 9.2 检查图片文件数量

```powershell
# PowerShell
Get-ChildItem "D:\FlightToolbox\miniprogram\packageWalkaroundImages*\images*" -Directory | ForEach-Object {
    $count = (Get-ChildItem $_.FullName -Filter *.png).Count
    Write-Host "$($_.Parent.Name)/$($_.Name): $count 张PNG"
}
```

### 9.3 验证预加载配置

```powershell
# PowerShell - 检查app.json的preloadRule
Get-Content "D:\FlightToolbox\miniprogram\app.json" |
    Select-String -Pattern '"preloadRule"' -Context 0,30
```

### 9.4 清理预加载状态（开发调试）

```javascript
// 微信开发者工具控制台
wx.setStorageSync('flight_toolbox_walkaround_preload_status', {})
console.log('✅ 预加载状态已清理')
```

---

## 10. 常见问题FAQ

### Q1: 为什么不直接使用wx.loadSubpackage手动加载？

**A**:
- wx.loadSubpackage需要用户主动触发（如点击按钮）
- 用户体验差（需要等待加载进度）
- preloadRule是微信后台自动加载，用户无感知
- 我们的引导系统只需要用户访问一次页面，之后永久有效

### Q2: 为什么不在TabBar页面配置preloadRule？

**A**:
- TabBar页面是应用启动时加载的首页
- 如果配置大量预加载，会显著延长启动时间
- 微信官方建议不在TabBar页面配置预加载

### Q3: 单页面预加载<2MB，是指分包大小还是图片大小？

**A**:
- 指的是**分包总大小**（包含图片、代码、配置文件等）
- 在"详情 → 本地代码 → 代码包大小详情"中查看
- 文件管理器显示的大小可能包含文件系统开销，以微信开发者工具为准

### Q4: 图片可以跨分包共享吗？

**A**:
- 不能共享，必须复制到每个需要的分包
- 允许重复存储（独立性原则）
- 虽然增加了总大小，但保证了分包独立加载的可靠性

### Q5: 添加新机型需要修改几个文件？

**A**:
- **数据文件**：3个（areas.js, checkitems.js, components.js）
- **配置文件**：2个（app.json, walkaround-preload-guide.js）
- **预加载页面**：N个（取决于分包数量，每个分包一个页面）
- **主页面**：1个（index.js，添加机型切换逻辑）

### Q6: 如何判断需要几个图片分包？

**A**:
```
公式：分包数 = ceil(总图片大小 / 1.8MB)

示例1：
- A330: 4.3MB ÷ 1.8MB = 2.4 → 需要3个分包
- 实际使用6个分包（更均衡）

示例2：
- B737: 3.5MB ÷ 1.8MB = 1.9 → 需要2个分包
- 建议使用4-5个分包（方便按区域划分）
```

**建议**：优先按区域范围均分，而不是完全按大小计算

### Q7: 预加载页面的选择有什么要求？

**A**:
1. 不能是TabBar页面
2. 用户访问频率较高（容易访问到）
3. 功能相关性（如燃油计算器 → B737燃油相关区域）
4. 单页面预加载总大小<2MB

**推荐选择**：
- packageO下的工具页面（日出日落、个人检查单等）
- 专业功能页面（体检标准、通信规范等）
- 避免选择复杂页面（如驾驶舱模块）

### Q8: 开发工具图片404，但真机正常，是否有问题？

**A**:
- **无问题**，这是正常现象
- 开发工具的分包加载机制与真机不同
- 以真机测试为准
- 如果真机也404，才需要排查问题

### Q9: 如何验证分包是否真的预加载了？

**A**:
1. 清理预加载状态
2. 打开微信开发者工具Network面板
3. 访问预加载页面（如日出日落）
4. 观察Network面板，应该看到分包下载请求
5. 返回绕机检查页面，点击对应区域
6. 图片请求应该显示"from cache"

### Q10: 清理预加载状态后，需要重新访问所有预加载页面吗？

**A**:
- **仅开发调试时需要**
- 真机用户使用时，预加载状态会永久保存（除非清除小程序缓存）
- 清理功能仅用于开发测试引导对话框

---

## 11. 总结

### 11.1 核心要点

1. ✅ **分包大小**：单个分包 < 2MB，实际控制在1.8MB以内
2. ✅ **预加载限制**：单页面预加载总大小 < 2MB
3. ✅ **8步配置法**：必须全部完成，缺一不可
4. ✅ **占位页面**：每个分包必须有pages/placeholder/
5. ✅ **路径规范**：绝对路径，前后都要斜杠
6. ✅ **真机测试**：以真机为准，开发工具仅供参考

### 11.2 添加新机型的最小步骤

```
1. 准备数据（3个数据文件 + Canvas图片）
2. 创建分包（N个图片分包目录 + 占位页面）
3. 配置app.json（subPackages + preloadRule）
4. 配置引导系统（walkaround-preload-guide.js）
5. 更新预加载页面（N个页面添加标记代码）
6. 更新主页面（机型切换逻辑 + 图片路径）
7. 编译验证（分包大小 + 功能测试）
8. 真机测试（Android + iOS + 离线模式）
```

### 11.3 下次添加B737直接使用

**准备阶段**：
- [ ] 收集B737检查区域数据（20个区域）
- [ ] 收集检查项数据（~100个检查项）
- [ ] 收集部件定义数据（~60个部件）
- [ ] 收集组件图片（~100张PNG）
- [ ] 准备B737 Canvas飞机图

**执行阶段**（直接按第4章步骤操作）：
- [ ] 创建data/b737/数据文件
- [ ] 创建5个图片分包
- [ ] 更新app.json（5个分包配置 + 5个preloadRule）
- [ ] 更新walkaround-preload-guide.js（5个区域映射）
- [ ] 更新5个预加载页面（添加标记代码）
- [ ] 更新主页面（机型切换 + 图片路径）
- [ ] 编译验证
- [ ] 真机测试

**预计耗时**：
- 数据准备：2-4小时
- 配置实施：1-2小时
- 测试验证：1小时
- **总计：4-7小时**（而不是今天的摸索时间）

---

**文档版本**: v2.0.0
**最后更新**: 2025-11-04
**作者**: 王航
**适用项目**: FlightToolbox（飞行工具箱）

**下次添加新机型时，直接打开本文档，按第4章步骤操作即可！**

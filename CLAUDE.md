...
cacheAudio: function(cacheKey, originalSrc) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    wx.getFileSystemManager().getFileInfo({
      ...
      success: function(res) {
        var targetPath = AUDIO_CACHE_DIR + '/' + cacheKey + '.mp3';
...
**关键差异**：
| 资源类型 | 获取信息API | 缓存索引Key | 文件扩展名 |
|---------|------------|------------|-----------|
| 图片 | `wx.getImageInfo` | `walkaround_image_cache_index` | `.png` / `.jpg` |
| 音频 | `wx.getFileSystemManager().getFileInfo` | `audio_cache_index` | `.mp3` |
| PDF | `wx.getFileInfo` | `pdf_cache_index` | `.pdf` |
...
| 资源类型 | 当前状态 | 适用场景 | 预估收益 | 实施难度 | 优先级 |
|---------|---------|---------|---------|---------|--------|
| 图片资源 | ✅ 已实现 | 绕机检查图片系统 | 🔥 高（离线稳定性↑80%） | 已完成 | P0 |
| 音频资源 | ⚠️ 可优化 | 31个音频分包 | 💡 中（减少"play audio fail"约80%） | 低（1-2天） | P1 |
| PDF文档 | 💡 可扩展 | ICAO出版物等 | 💬 低（PDF较小，加载较稳定） | 中（2-3天） | P3 |
| 视频资源 | 💡 可扩展 | 培训视频等 | 💬 低（视频较少使用） | 高（需考虑存储） | P4 |

**收益说明**：
- **离线稳定性**：二次启动时资源可用率（无缓存 vs 有缓存）
- **性能提升**：加载速度提升倍数
- **用户体验**：减少加载失败提示和重新预加载次数
...
| 组件 | 大小 | 说明 |
|------|------|------|
| 缓存索引（JSON对象） | 1-5KB | 存储在Storage中，可忽略不计 |
| 单张图片缓存 | 100-300KB | PNG格式，视图片内容而定 |
| Promise管理器 | <1KB | 仅在缓存写入时临时占用 |
| 文件系统句柄 | <1KB | wx.getFileSystemManager()实例 |

**总内存占用**：约10-15MB（绕机检查54张图片）

...
该特性需要基础库版本 [2.11.2](../../../analysis/experience/compatibility.html) 或以上，使用该特性的小程序在 2.11.2 以下的基础库上可能无法工作，如果发布正式版本，可以考虑将最低基础库版本设置为 2.11.2 或以上。
...
一个分包使用其他分包的自定义组件时，由于其他分包还未下载或注入，其他分包的组件处于不可用的状态。通过为其他分包的自定义组件设置 [占位组件](../custom-component/placeholder.html)，我们可以先渲染占位组件作为替代，在分包下载完成后再进行替换。例如：
...
在基础库 `2.24.3` 之后，可以使用 [`wx.onLazyLoadError`](../../api/base/app/app-event/wx.onLazyLoadError) 监听加载事件。
...
详情可参考 [require 文档](../../reference/api/require)

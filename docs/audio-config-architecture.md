# 音频配置架构设计文档

## 📖 概述

本文档描述了 FlightToolbox 小程序中音频配置管理系统的架构设计，旨在提供一个可扩展、易维护的音频资源管理方案。

## 🎯 设计目标

- **可扩展性**：轻松添加新的地区和机场
- **可维护性**：单一数据源，避免重复配置
- **职责分离**：配置逻辑与数据分离
- **向后兼容**：不影响现有功能

## 🏗️ 架构设计

### 文件结构

```
miniprogram/
├── utils/
│   ├── audio-config.js          # 音频配置管理器（核心）
│   └── communication-rules.js   # 通信规则配置
├── data/
│   ├── regions/
│   │   ├── japan.js             # 日本音频数据
│   │   ├── philippines.js       # 菲律宾音频数据
│   │   ├── korean.js            # 韩国音频数据
│   │   ├── germany.js           # 德国音频数据（未来）
│   │   └── usa.js               # 美国音频数据（未来）
│   └── index.js                 # 数据统一导出
├── packageJ/                    # 日本音频分包
├── packageK/                    # 菲律宾音频分包
├── packageL/                    # 韩国音频分包
└── packageM,N,O,P...           # 其他音频分包
```

### 核心组件

#### 1. 音频配置管理器 (`utils/audio-config.js`)

**职责：**
- 统一管理所有音频配置
- 提供标准化的数据访问接口
- 处理音频路径映射
- 管理地区和机场信息

**主要方法：**
```javascript
class AudioConfigManager {
  getRegions()                    // 获取所有地区
  getAirports()                   // 获取所有机场
  getAirportsByRegion(regionId)   // 根据地区获取机场
  getAirportById(airportId)       // 根据ID获取机场
  getAudioPath(airportId, filename) // 获取音频文件路径
  getFullConfig()                 // 获取完整配置
}
```

#### 2. 数据文件 (`data/regions/*.js`)

**职责：**
- 存储纯净的音频数据
- 每个地区一个独立文件
- 统一的数据格式

**数据格式：**
```javascript
const RegionData = {
  clips: [
    {
      label: "进近",                    // 录音类型
      full_transcript: "...",          // 英文原文
      translation_cn: "...",           // 中文翻译
      mp3_file: "xxx.mp3"              // 音频文件名
    }
  ]
};

module.exports = RegionData;
```

#### 3. 数据导出器 (`data/index.js`)

**职责：**
- 统一导出所有地区数据
- 便于未来自动化加载

## 🚀 使用方法

### 在页面中使用

```javascript
// 加载配置
const audioConfig = require('../../utils/audio-config.js');
const config = audioConfig.airlineRecordingsData;

// 使用配置管理器
const manager = audioConfig.audioConfigManager;
const regions = manager.getRegions();
const airports = manager.getAirportsByRegion('asia');
const audioPath = manager.getAudioPath('japan', 'sample.mp3');
```

### API 接口

```javascript
// 获取所有地区
const regions = audioConfigManager.getRegions();
// 返回: [{ id, name, icon, color, description }, ...]

// 获取地区下的机场
const airports = audioConfigManager.getAirportsByRegion('asia');
// 返回: [{ id, regionId, name, city, icao, clips }, ...]

// 获取音频路径
const path = audioConfigManager.getAudioPath('japan', 'sample.mp3');
// 返回: "/packageJ/sample.mp3"
```

## 📈 扩展指南

### 添加新地区

#### 1. 创建数据文件

```bash
# 创建欧洲地区数据文件
touch miniprogram/data/regions/europe.js
```

```javascript
// miniprogram/data/regions/europe.js
const EuropeData = {
  clips: [
    {
      label: "进近",
      full_transcript: "Lufthansa 441, turn left heading 270",
      translation_cn: "汉莎441，左转航向270",
      mp3_file: "LH441_Turn-Left-270.mp3"
    }
    // ... 更多录音
  ]
};

module.exports = EuropeData;
```

#### 2. 更新配置管理器

```javascript
// miniprogram/utils/audio-config.js
const europeData = require('../data/regions/europe.js');

// 在 constructor 中添加地区
this.regions.push({
  id: 'europe',
  name: '欧洲',
  icon: '🌍',
  color: '#10B981',
  description: '欧洲地区机场陆空通话录音'
});

// 添加机场
this.airports.push({
  id: 'frankfurt',
  regionId: 'europe',
  name: '德国法兰克福机场',
  city: '法兰克福',
  icao: 'EDDF',
  packageName: 'packageL',
  audioPath: '/packageL/',
  icon: '🏰',
  description: '法兰克福国际机场陆空通话录音',
  clips: europeData.clips || []
});
```

#### 3. 创建音频分包

```bash
mkdir miniprogram/packageL
# 将音频文件放入 packageL 目录
```

#### 4. 更新小程序配置

```json
// miniprogram/app.json
{
  "subPackages": [
    {
      "root": "packageL",
      "name": "europeAudioPackage",
      "pages": ["index"]
    }
  ]
}
```

### 自动化脚本（建议）

```bash
#!/bin/bash
# add-region.sh - 自动添加新地区脚本

REGION_ID=$1
REGION_NAME=$2
PACKAGE_NAME=$3

# 创建数据文件
cat > "miniprogram/data/regions/${REGION_ID}.js" << EOF
const ${REGION_ID^}Data = {
  clips: [
    // TODO: 添加录音数据
  ]
};

module.exports = ${REGION_ID^}Data;
EOF

# 创建分包目录
mkdir -p "miniprogram/${PACKAGE_NAME}"

echo "✅ 已创建 ${REGION_NAME} 地区的基础文件"
echo "📝 请手动更新 utils/audio-config.js 添加配置"
```

## 🔄 迁移指南

### 从旧架构迁移

如果您的项目使用旧的配置方式：

1. **备份现有文件**
2. **迁移数据文件**
3. **更新引用路径**
4. **测试功能完整性**

### 旧配置清理

```bash
# 删除重复的配置文件
rm miniprogram/utils/AirlineRecordings.js
rm miniprogram/utils/japan-audio-config.js
rm miniprogram/utils/philippine-audio-config.js
```

## 🛠️ 最佳实践

### 1. 数据格式标准化

确保所有地区数据文件遵循统一格式：

```javascript
// 标准格式
const RegionData = {
  clips: [
    {
      label: "录音类型",           // 必需：进近/地面/塔台/放行
      full_transcript: "英文原文", // 必需：完整英文录音文本
      translation_cn: "中文翻译",  // 必需：中文翻译
      mp3_file: "文件名.mp3"      // 必需：音频文件名
    }
  ]
};
```

### 2. 命名规范

- **地区ID**：小写英文，如 `japan`, `europe`, `usa`
- **机场ID**：小写英文，如 `tokyo`, `frankfurt`, `newyork`
- **分包名**：`package` + 字母，如 `packageJ`, `packageK`
- **音频文件**：`航班号_描述.mp3`，如 `LH441_Turn-Left-270.mp3`

### 3. 性能优化

- 使用分包预加载策略
- 音频文件大小控制在 500KB 以内
- 按需加载音频数据

### 4. 错误处理

```javascript
// 示例：安全的配置获取
try {
  const audioPath = audioConfigManager.getAudioPath(airportId, filename);
  if (!audioPath) {
    console.warn(`音频路径获取失败: ${airportId}/${filename}`);
    return;
  }
  // 使用音频路径
} catch (error) {
  console.error('音频配置错误:', error);
  // 降级处理
}
```

## 📊 架构优势

### ✅ 优势

1. **单一数据源**：避免数据重复和不一致
2. **职责清晰**：配置管理与数据存储分离
3. **易于扩展**：添加新地区只需2-3个步骤
4. **便于维护**：修改数据只需更新一个文件
5. **自动化友好**：支持脚本化批量操作
6. **类型安全**：统一的数据接口和格式验证

### 📈 扩展性对比

| 操作 | 旧架构 | 新架构 |
|------|--------|--------|
| 添加新地区 | 修改3-5个文件 | 创建1个数据文件 + 更新配置 |
| 修改录音数据 | 同步多个文件 | 修改1个数据文件 |
| 维护复杂度 | 高（多文件同步） | 低（单一配置源） |
| 出错概率 | 高（人工同步） | 低（自动化管理） |

## 🔍 故障排除

### 常见问题

1. **音频加载失败**
   - 检查分包路径配置
   - 验证音频文件是否存在
   - 确认分包预加载设置

2. **数据格式错误**
   - 检查 `module.exports` 语法
   - 验证数据结构完整性
   - 确认文件编码格式

3. **配置更新不生效**
   - 重启开发者工具
   - 清除小程序缓存
   - 检查文件引用路径

## 📝 更新日志

### v1.0.0 (2024-01-XX)
- 初始架构设计
- 支持日本、菲律宾地区
- 基础配置管理器实现

### v1.1.0 (2024-07-05)
- ✅ 添加韩国地区支持 (packageL)
- ✅ 包含20个真实仁川机场录音
- ✅ 更新分包配置和预加载规则
- 调整德国分包路径为packageP

### v1.2.0 (计划中)
- 添加更多欧洲地区支持
- 自动化脚本工具
- 性能优化

---

**文档维护者**：FlightToolbox 开发团队  
**最后更新**：2024-01-XX  
**版本**：1.0.0
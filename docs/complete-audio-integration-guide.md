# 🛩️ FlightToolbox 完整音频集成指南

## 📖 概述

这是 FlightToolbox 小程序音频系统的**完整技术指南**，涵盖新增音频、预加载配置、用户引导等全部功能。任何开发者按照此文档，都能快速理解并扩展音频系统。

## 🎯 系统架构总览

### 核心组件架构
```
FlightToolbox 音频系统
├── 数据层 (data/regions/*.js)          # 音频数据和元信息
├── 配置层 (utils/audio-config.js)      # 统一配置管理器
├── 分包层 (package*/）                 # 音频资源分包存储
├── 预加载层 (app.json preloadRule)     # 分包预加载策略
├── 引导层 (utils/audio-preload-guide.js) # 用户引导系统
├── 界面层 (pages/*)                    # 用户交互界面
└── 播放层 (audio-player)               # 音频播放核心
```

### 关键技术决策

#### 🚨 核心发现：预加载是音频真机播放的关键
**经过实际测试验证**：
- ✅ **预加载分包**：真机播放成功率 100%，支持完全离线播放
- ❌ **异步加载分包**：真机播放成功率低，飞行模式下完全失效

**🎯 结论**：所有音频分包必须通过预加载实现，避免异步加载的不稳定性

#### 分散预加载策略
- **2MB限制**：微信小程序单页面预加载不能超过2MB
- **突破方案**：利用多个常用页面分散预加载，实现全覆盖
- **离线优先**：确保飞行员在空中（飞行模式）能正常使用

---

## ⚡ 新增音频完整流程

### 第1步：准备音频数据文件 📁

在 `miniprogram/data/regions/` 目录下创建新的数据文件：

**文件名格式**：`[国家英文小写].js`

**示例**：`singapore.js`

```javascript
const SingaporeData = {
  "clips": [
    {
      "label": "进近",
      "full_transcript": "Singapore Airlines 117, turn left heading 180, descend to 9000 feet.",
      "translation_cn": "新航117，左转航向180，下降至9000英尺。",
      "mp3_file": "Singapore-Airlines-117_Turn-Left-180-Descend-9000.mp3"
    },
    {
      "label": "塔台",
      "full_transcript": "Singapore Airlines 117, runway 02 left, cleared to land.",
      "translation_cn": "新航117，跑道02左，可以落地。",
      "mp3_file": "Singapore-Airlines-117_Runway-02L-Cleared-to-Land.mp3"
    }
    // ... 更多录音数据
  ]
};

module.exports = SingaporeData;
```

**⚠️ 数据格式要求**：
- `label`：录音类型（进近/塔台/地面/放行/区调）
- `full_transcript`：英文原文（必须准确）
- `translation_cn`：中文翻译
- `mp3_file`：音频文件名（格式：`航班号_动作描述.mp3`）

---

### 第2步：创建音频分包目录 📦

在 `miniprogram/` 目录下创建分包文件夹：

**命名格式**：`package[国家英文首字母大写]`

```bash
# 创建新加坡分包目录
mkdir miniprogram/packageSingapore

# 将所有 MP3 音频文件放入该目录
cp *.mp3 miniprogram/packageSingapore/
```

**必须创建的基础文件**：

1. **index.js**：
```javascript
Page({
  data: {},
  onLoad: function() {
    console.log('新加坡音频分包页面加载');
  }
});
```

2. **index.json**：
```json
{
  "navigationBarTitleText": "新加坡机场录音",
  "navigationBarBackgroundColor": "#16a085",
  "navigationBarTextStyle": "white"
}
```

3. **index.wxml**：
```xml
<view class="container">
  <view class="header">
    <text>🇸🇬 新加坡樟宜机场录音</text>
  </view>
</view>
```

---

### 第3步：更新音频配置管理器 ⚙️

编辑 `miniprogram/utils/audio-config.js` 文件：

#### 3.1 添加数据文件引用

在文件顶部的 `try` 块中添加：

```javascript
// 找到这段代码
try {
  japanData = require('../data/regions/japan.js');
  philippinesData = require('../data/regions/philippines.js');
  koreanData = require('../data/regions/korean.js');
  // 👇 添加这一行
  singaporeData = require('../data/regions/singapore.js');
  // ... 其他数据文件
}
```

#### 3.2 添加地区信息

在 `this.regions` 数组中添加：

```javascript
{
  id: 'singapore',
  continentId: 'asia',
  name: '新加坡',
  flag: '🇸🇬',
  description: '樟宜机场真实陆空通话录音',
  count: 8,  // 实际录音数量
  hasRealRecordings: true
},
```

#### 3.3 添加机场配置

在 `this.airports` 数组中添加：

```javascript
{
  id: 'singapore',
  regionId: 'singapore',
  name: '新加坡樟宜机场',
  city: '新加坡',
  icao: 'WSSS',
  packageName: 'packageSingapore',
  audioPath: '/packageSingapore/',
  icon: '🌟',
  description: '樟宜国际机场陆空通话录音',
  clips: singaporeData.clips || []
},
```

---

### 第4步：配置分包预加载策略 🎯

这是**最关键的步骤**！预加载配置决定了音频是否能在真机上正常播放。

#### 4.1 添加分包定义

编辑 `miniprogram/app.json` 文件，在 `subPackages` 数组中添加：

```json
{
  "root": "packageSingapore",
  "name": "singaporeAudioPackage",
  "pages": ["index"]
},
```

#### 4.2 计算分包大小

```bash
# 检查新分包大小
du -sh miniprogram/packageSingapore/

# 检查所有分包大小
du -sh miniprogram/package*/
```

#### 4.3 选择预加载页面

**预加载页面选择原则**：
1. **用户访问频率**：优先选择用户高频访问的页面
2. **剩余容量**：确保该页面预加载总大小 < 2MB
3. **功能关联性**：相关功能的数据放在同一页面

**当前预加载策略（2025年7月）**：

```json
"preloadRule": {
  "pages/home/index": {
    "network": "all",
    "packages": ["packageKorean", "packageAustralia"]
  },
  "pages/search/index": {
    "network": "all",
    "packages": ["packageA", "packageB", "packageAustralia"]
  },
  "pages/operations/index": {
    "network": "all",
    "packages": ["packagePhilippines", "packageSingapore", "packageAustralia"]
  },
  "pages/air-ground-communication/index": {
    "network": "all",
    "packages": ["packagePhilippines", "packageSingapore"]
  },
  "pages/recording-categories/index": {
    "network": "all",
    "packages": ["packageRussia"]
  },
  "pages/recording-clips/index": {
    "network": "all",
    "packages": ["packageSrilanka"]
  },
  "pages/audio-player/index": {
    "network": "all",
    "packages": ["packageJapan"]
  },
  "pages/communication-rules/index": {
    "network": "all",
    "packages": ["packageE"]
  },
  "pages/airline-recordings/index": {
    "network": "all",
    "packages": ["packageH", "packageAmerica"]
  },
  "packageO/sunrise-sunset/index": {
    "network": "all",
    "packages": ["packageTurkey"]
  },
  "pages/communication-failure/index": {
    "network": "all",
    "packages": ["packageItaly"]
  },
  "pages/performance-parameters/index": {
    "network": "all",
    "packages": ["packagePerformance", "packageF", "packageO"]
  },
  "pages/medical-standards/index": {
    "network": "all",
    "packages": ["packageUAE"]
  },
  "packageO/personal-checklist/index": {
    "network": "all",
    "packages": ["packageThailand"]
  },
  "packageO/flight-time-share/index": {
    "network": "all",
    "packages": ["packageFrance"]
  }
}
```

#### 4.4 添加新分包到预加载

**示例**：将新加坡分包添加到航班运行页面

```json
"pages/air-ground-communication/index": {
  "network": "all",
  "packages": ["packagePhilippines", "packageSingapore"]  // 添加新分包
}
```

---

### 第5步：更新音频播放器配置 🎵

编辑 `miniprogram/pages/audio-player/index.ts` 文件：

#### 5.1 添加路径映射

在 `regionPathMap` 对象中添加：

```typescript
const regionPathMap: { [key: string]: string } = {
  'japan': '/packageJapan/',
  'philippines': '/packagePhilippines/', 
  'korea': '/packageKorean/',
  'singapore': '/packageSingapore/',  // 👈 必须添加这一行
  'thailand': '/packageThailand/',
  'russia': '/packageRussia/',
  'srilanka': '/packageSrilanka/',
  'turkey': '/packageTurkey/',
  'uae': '/packageUAE/',
  'france': '/packageFrance/',
  'usa': '/packageAmerica/',
  'italy': '/packageItaly/',
  'australia': '/packageAustralia/',
  // ... 其他路径映射
};
```

---

### 第6步：更新预加载引导配置 🎯

编辑 `miniprogram/utils/audio-preload-guide.js` 文件：

在 `preloadPageMapping` 对象中添加新地区的引导配置：

```javascript
'singapore': {
  packageName: 'packageSingapore',
  regionName: '新加坡樟宜机场',
  flag: '🇸🇬',
  preloadPage: 'pages/air-ground-communication/index',
  preloadPageName: '航班运行',
  preloadPageIcon: '✈️',
  description: '新加坡樟宜机场陆空通话录音将通过航班运行页面自动预加载'
},
```

---

### 第7步：语法检查和测试 ✅

#### 7.1 语法检查

```bash
# JavaScript语法检查（ES5验证）
node -c miniprogram/data/regions/singapore.js
node -c miniprogram/utils/audio-config.js
node -c miniprogram/utils/audio-preload-guide.js

# 检查JSON语法
node -e "require('./miniprogram/app.json')"
```

#### 7.2 功能测试

**开发者工具测试**：
1. 重启开发者工具
2. 检查控制台无错误
3. 进入"航线飞行"页面
4. 选择新增国家
5. 验证音频播放功能

**真机测试**：
1. 扫码预览到真机
2. 测试音频播放功能
3. 验证离线播放能力

---

## 🎯 预加载引导系统

### 工作原理

当用户尝试播放未预加载的音频时，系统会：

1. **检测播放失败**：`operateAudio:fail:play audio fail`
2. **显示引导对话框**：告知用户需要访问哪个页面
3. **一键跳转**：直接跳转到预加载页面
4. **自动重检**：返回后自动检查预加载状态

### 引导对话框示例

```
🇸🇬 新加坡樟宜机场 音频资源

要播放新加坡樟宜机场的陆空通话录音，需要先预加载音频资源。

请点击下方按钮访问 ✈️ 航班运行 页面，系统将自动为您预加载音频资源。

[前往] [稍后再说]
```

### 关键实现细节

#### 错误检测逻辑
```javascript
// 检测分包加载错误
const isSubpackageError = error.errCode === 10001 || 
                         error.errMsg.includes('not found param') ||
                         error.errMsg.includes('play audio fail');

// 对于播放失败，直接显示引导
if (error.errMsg.includes('play audio fail')) {
  this.showPreloadGuideDialog(this.data.currentClip);
  return;
}
```

#### 微信小程序限制
- **confirmText** 不能超过4个中文字符
- **content** 内容建议控制在200字符以内

---

## 📊 当前系统状态

### 已集成地区（13个音频分包）

| 国家/地区 | 分包名 | 录音数量 | 预加载页面 | 状态 |
|-----------|--------|----------|------------|------|
| 🇯🇵 日本 | `packageJapan` | 24个 | 航线录音 | ✅ 完成 |
| 🇵🇭 菲律宾 | `packagePhilippines` | 27个 | 航班运行 | ✅ 完成 |
| 🇰🇷 韩国 | `packageKorean` | 19个 | 我的首页 | ✅ 完成 |
| 🇸🇬 新加坡 | `packageSingapore` | 8个 | 航班运行 | ✅ 完成 |
| 🇹🇭 泰国 | `packageThailand` | 22个 | 航线录音/个人检查单 | ✅ 完成 |
| 🇷🇺 俄罗斯 | `packageRussia` | 23个 | 录音分类 | ✅ 完成 |
| 🇱🇰 斯里兰卡 | `packageSrilanka` | 22个 | 录音片段 | ✅ 完成 |
| 🇦🇺 澳大利亚 | `packageAustralia` | 20个 | 资料查询/航班运行/我的首页 | ✅ 优化完成 |
| 🇹🇷 土耳其 | `packageTurkey` | 28个 | 日出日落 | ✅ 完成 |
| 🇫🇷 法国 | `packageFrance` | 19个 | 飞行时间分享 | ✅ 完成 |
| 🇺🇸 美国 | `packageAmerica` | 52个 | 音频播放器 | ✅ 完成 |
| 🇮🇹 意大利 | `packageItaly` | 29个 | 通信失效程序 | ✅ 完成 |
| 🇦🇪 阿联酋 | `packageUAE` | 38个 | 体检标准 | ✅ 完成 |

### 总计统计
- **音频分包**：13个
- **录音总数**：**331条** 真实机场录音
- **覆盖国家**：13个主要航空国家
- **预加载覆盖率**：100%（所有分包都已预加载）

---

## 🔧 常见问题解决

### Q1：音频播放失败
**错误信息**：`operateAudio:fail:play audio fail`
**原因**：音频分包未预加载或路径配置错误
**解决**：
1. 检查预加载引导是否显示
2. 按引导访问对应页面
3. 检查 `regionPathMap` 配置

### Q2：预加载超限
**错误信息**：`preloadRule source size exceed max limit 2MB`
**原因**：页面预加载分包总大小超过2MB
**解决**：
1. 重新分配分包到其他页面
2. 优化音频文件大小
3. 拆分大分包为多个小分包

### Q3：引导对话框不显示
**错误信息**：`showModal:fail confirmText length should not larger than 4 Chinese characters`
**原因**：对话框按钮文字超过4个中文字符限制
**解决**：缩短 `confirmText` 文字

### Q4：真机播放失败但开发者工具正常
**原因**：分包未正确预加载，开发者工具环境宽松
**解决**：
1. 确保分包已配置预加载
2. 检查预加载页面配置
3. 在真机上测试预加载功能

### Q5：澳大利亚音频无法播放（已修复）
**问题描述**：其他国家音频都能播放，只有澳大利亚的无法播放
**根本原因**：`app.json` 中缺少澳大利亚音频分包的预加载配置
**修复方案**：
1. 在 `app.json` 的 `preloadRule` 中添加：
```json
"pages/search/index": {
  "network": "all",
  "packages": ["packageA", "packageB", "packageAustralia"]
}
```
2. 确保 `audio-preload-guide.js` 中的映射配置正确
3. 验证 `audio-player/index.ts` 中的路径映射包含 `'australia': '/packageAustralia/'`

---

## 🎯 最佳实践

### 1. 音频文件优化
- **格式**：MP3格式，推荐码率128kbps
- **大小**：单个文件控制在500KB以内
- **命名**：使用英文，格式 `航班号_动作描述.mp3`

### 2. 预加载策略
- **优先预加载**：所有新音频分包必须配置预加载
- **容量平衡**：确保每个页面预加载 < 2MB
- **用户体验**：选择用户高频访问的页面

### 3. 开发流程
- **ES5语法**：严格使用ES5语法，避免ES6+特性
- **语法检查**：使用 `node -c` 验证所有JS文件
- **真机测试**：关键功能必须在真机上验证

### 4. 错误处理
- **用户友好**：提供清晰的错误提示和解决方案
- **引导机制**：自动引导用户解决预加载问题
- **兜底方案**：提供多种解决途径

---

## 📋 快速检查清单

### 新增音频分包检查清单
- [ ] 创建数据文件：`data/regions/[country].js`
- [ ] 创建分包目录：`package[Country]/`
- [ ] 添加基础页面文件：`index.js`, `index.json`, `index.wxml`
- [ ] 更新音频配置：`utils/audio-config.js`
- [ ] 配置分包定义：`app.json` subPackages
- [ ] 配置预加载策略：`app.json` preloadRule
- [ ] 添加路径映射：`audio-player/index.ts`
- [ ] 配置预加载引导：`utils/audio-preload-guide.js`
- [ ] 语法检查：`node -c` 验证所有文件
- [ ] 功能测试：开发者工具和真机测试

### 预加载配置检查清单
- [ ] 分包大小：`du -sh package*/` 检查所有分包
- [ ] 预加载总量：确保每页面 < 2MB
- [ ] 引导配置：检查用户引导映射
- [ ] 真机测试：验证离线播放能力

---

## 🚀 未来扩展

### 添加新音频分包标准流程

1. **准备阶段**：
   - 收集音频文件和转录文本
   - 确定国家和机场信息
   - 评估分包大小

2. **开发阶段**：
   - 按本文档流程完成所有配置
   - 进行充分的语法检查
   - 完成功能测试

3. **测试阶段**：
   - 开发者工具测试
   - 真机测试（必须）
   - 离线播放测试

4. **部署阶段**：
   - 更新相关文档
   - 发布版本
   - 用户反馈收集

### 系统扩展方向

- **多语言支持**：支持更多国家的音频资源
- **智能预加载**：基于用户行为优化预加载策略
- **音质优化**：提升音频质量和压缩效率
- **用户个性化**：个性化音频内容推荐

---

## 📝 更新日志

### v3.2 - 2025年7月26日
- **优化**：澳大利亚音频预加载策略
- **问题**：用户从航线录音页面无法播放澳大利亚音频
- **原因**：澳大利亚分包仅在资料查询页面预加载，覆盖不足
- **方案**：
  - 新增 `pages/operations/index`（航班运行）预加载澳大利亚分包
  - 新增 `pages/home/index`（我的首页）预加载澳大利亚分包
  - 更新预加载引导页面为航班运行
- **状态**：✅ 已优化

### v3.1 - 2025年7月25日
- **修复**：澳大利亚音频无法播放的问题
- **原因**：`app.json` 中缺少澳大利亚分包的预加载配置
- **方案**：在 `pages/search/index` 的预加载规则中添加 `packageAustralia`
- **状态**：✅ 已修复

### v3.0 - 2025年7月18日
- **新增**：完整的音频集成技术指南
- **优化**：预加载引导系统
- **更新**：13个国家的音频分包配置

---

**文档版本**：v3.2  
**最后更新**：2025年7月26日  
**维护者**：FlightToolbox 开发团队  
**适用版本**：微信小程序基础库 2.0+

---

✈️ **祝您顺利完成音频集成！**

> 这份文档整合了音频配置、预加载策略、用户引导等全部功能，确保其他AI或开发者能够快速理解和扩展FlightToolbox的音频系统。
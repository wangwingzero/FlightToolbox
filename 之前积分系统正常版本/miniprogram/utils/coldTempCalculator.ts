 ...
 * 计算函数的输入参数接口
 */
export interface ColdTempInput {
  airportElevationFeet: number;   // 机场标高 (英尺)
  airportTemperatureC: number;    // 机场温度 (°C)
  uncorrectedAltitudeFeet: number; // 需要修正的修正前高度 (英尺)
  
  // --- FPA 计算所需的可选参数 ---
  isFafPoint?: boolean;                    // 标志位，表示是否为FAF点计算
  fafDistanceNm?: number;                  // FAF点到跑道头的水平距离 (海里)
}
...
export interface CorrectionResult {
  correctionFeet: number;         // 计算出的高度修正值 (英尺)
  correctedAltitudeFeet: number;  // 修正后的高度 (英尺)
  
  // --- VPA 相关结果 ---
  vpaInfo?: {
    designedVpa: number;          // 设计的VPA
    actualVpa: number;            // 未修正情况下的实际VPA
    vpaChange: number;            // VPA 变化量
 ...
 */
export function calculateColdTempCorrection(input: ColdTempInput): CorrectionResult {
  const { 
